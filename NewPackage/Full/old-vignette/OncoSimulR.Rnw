...
for instance, in Oncogenetic Tree (OT; \citealp{Desper1999JCB, Szabo2008})
or Conjunctive Bayesian Network (CBN; \citealp{Beerenwinkel2007,
  Gerstung2009, Gerstung2011}) models. Moreover, you can add passenger
mutations to the simulations. The models so far implemented are all
continuous time models, which are simulated using the BNB algorithm of
\cite{Mather2012}. This is a summary of some of the key features:
...
\item Right now, three different models are available, two that lead to
  exponential growth, one of them loosely based on \cite{Bozic2010}, and
  another that leads to logistic-like growth, based on \cite{McFarland2013}.
...
\texttt{graph} for the vignette to work (you do not need that for your
...
\texttt{poset}. It is often useful, to make sure you did not make any
...
very small \textit{initSize}. Likewise, most of the Bozic runs do not
...
infer an oncogenetic tree (and plot it) using the \texttt{Oncotree} package
\citep{Oncotree} after getting a quick look at the marginal frequencies of
events:
...
%% but it seems reftex-create-bibtex-file does the same or similar from emacs?
%% rm OncoSimulR.bib
