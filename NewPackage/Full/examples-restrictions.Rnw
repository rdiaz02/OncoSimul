...
\subsubsection{A conjunction example}\label{cbn1}

Let's try a first attempt
<<>>=
...
deviations from the poset (see discussion in \cite{xx: paper BMCBionfo}).
...
CBN and the monotone relationship (see sections \ref{cbn1} and
...
We also see a very different pattern compared to CBN (section \ref{cbn1})
here:
<<>>=
gfx1[28, ] 
1.01 * 1.1 * 1.2
...
This is like a revert pattern with respect to synthetic viability \ref{sv}.
\begin{tabular} {c c c}
  A & B & Fitness \\
  \hline
  wt&wt& 1 \\
...
effects. See also section \ref{theminus} and \ref{e3}.
...
should not be there. Otherwise, you need to carefully add coefficients.
...
\item Synthetic mortality between modules ``I'' (already present in the
  epistatic interaction) and ``K'' (with genes ``k1'' and ``k2''): the
  joint presence of these modules leads to cell death (fitness of 0).
\item Synthetic viability between modules ``L'' and ``M'' (with genes
  ``l1'', ``l2'' and ``m1'', respectively), so that only their joint
  presence is viable but adds nothing to fitness (i.e., mutation of both
  has fitness $1$, but of each one without the other leads to fitness of
  0).
...
As we are specifying very many different things, we will start by writing
each set of effects separately:
...
Yes, do it: as -p,s1, and -p,s2, etc.
