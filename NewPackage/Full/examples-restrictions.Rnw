...
\subsubsection{A conjunction example}
...
That is an error because the ``sh'' varies within a child, and that is not
possible (arbitrary values are better coded using order effects as in
section \ref{oe} and epistasis as in section \ref{epi}).
...
each of ``a'', ``b'', and ``c'' are present or not. 

<<>>=
evalAllGenotypes(fc1, order = FALSE)
@ 
...
<<>>=
evalAllGenotypes(fc1, order = TRUE)
@ 
...
\subsection{Synthetic mortality}
