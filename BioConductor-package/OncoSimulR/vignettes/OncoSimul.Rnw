...
\subsection{Numeric values of fitness effects: no viability and models}

FIXME

For all models where fitness affects directly the birth rate (for now, all
except Bozic), if you specify some ``s = -1'', if that event happens
fitness becomes immediately zero, and that clone does not even get
created. It is not viable.

Models based on Boziz, however, have a birth rate of 1 (beware, not
bozic2!) and a death rate that is a product blablabla.

In this case, a death rate larger than birth rate, per se, does not signal
immediate extinction and, moreover, even for death rates that are a few
times larger than birth rates, the clone could mutate before becoming
extinct\footnote{We said ``a few times''. For a clone of population size 1
  ...
  before becoming extinct is essentially zero for all reasonable values of
  mutation rate}. How do we signal immediate extinction, no viability? You
can set the value of $s = -\infty$. Setting a value of $s < -90$ has the
same effect.

So, if you want to identify some mutations or some combinations of
mutations as leading to immediate extinction, no viability, of the
affected clone, set ``s'' to some value $<= -90$, or set it to $-\infty$.


Most examples below evaluate fitness by its effects on the birth rate. You
can see one where we do it both ways in Section \ref{fit-neg-pos}.
...
FIXME: say something about the CBN, OR and XOR models.
...
\subsubsection{Synthetic viability and using Bozic model}\label{fit-neg-pos}
...
\subsection{McFl parameterization}\label{mcfl}


In the original McFarland model zz.  As I have $(1 + s)$, the new
$s = -s_p/(1 + s_p)$ (see in Chris' thesis, p. 9, in eq. 2.1)
...
tmp <-  oncoSimulIndiv(examplesFitnessEffects$cbn1,
                       model = "McFL", 
                       mu = 5e-7,
                       detectionSize = 1e8, 
                       detectionDrivers = 2,
...
@ 
Interesting, because lots of clonal interference and num drivers says
little, as fitness is very different and does not increase simply and a
lot with drivers:
...
But in other configurations, we do not reach the three gene mutant and
continue with clone competition for a long time:
...
(Instead of specifying ``extraTime'' we can set the detectionDrivers to
larger than the number of existing possible drivers, and it will run until
finalTime.)

FIXME: plot a popSimul
