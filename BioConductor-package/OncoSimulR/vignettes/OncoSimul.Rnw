...
several models of tumor progression with emphasis on restriction the
accumulation of mutations as specified, for example, in Oncogenetic Tree
(OT; \cite{Desper1999JCB, Szabo2008}) or Conjunctive Bayesian Network
(CBN; \cite{Beerenwinkel2007, Gerstung2009, Gerstung2011})
models. Moreover, you could add passenger mutations to the simulations.


Since then, OncoSimulR has been vastly extended to allow you to specify,
also, other types of restriction in the accumulation of genes, as specified
by the ``semimonotone'' model of \cite{FarahaniLagergren2013} and the XOR
models of \cite{Korsunsky2014}. 

Moreover, different fitness effects related to the order in which
mutations appear can also be incorporated, involving arbitrary numbers of
genes. This is different from ``restrictions in the accumulation of
mutations''. In this phenomenon, shown empirically in a recent paper
\cite{Ortmann2015}, the effect of having both mutations ``A'' and ``B''
differs depending on whether ``A'' appeared before or after
``B''. OncoSimulR also allows now to specify arbitrary epistatic
...
or show order effects with other genes).

Moreover, it is possible to specify the above interactions in terms of
modules, not genes. This idea is discussed in, for example,
\cite{Raphael2014a, Gerstung2011}: the restrictions encoded in, say, CBNs
or OT can be considered to apply not to genes, but to modules, where each
module is a set of genes (and the intersection between modules is the
empty set). Modules, then, play the role of a ``union operation'' over
sets of genes.


Finally, arbitrary numbers of genes without interactions (and with fitness
effects coming from any distribution you might want) are also possible.
...
core of the code is implemented in C++, providing for fast
execution. Finally, code to simulate random graphs of the kind often seen
in CBN, OTs, etc, has also been added.


Therefore, OncoSimulR is now a very general package for genetic
simulation, with applicability well beyond tumor progression.
...
can be found in \cite{Diaz-Uriarte2015}, where additional comments about
model parameters and caveats are discussed. The Java program by
\cite{Reiter2013a} offers somewhat similar functionality to the previous
version of OncoSimulR, but it is restricted to at most four drivers, you
cannot use arbitrary CBNs or OTs or XORs or semimonotone graphs to specify
order restrictions, there is no allowance for passengers, and a single
type of model (a discrete time Galton-Watson process) is implemented. The
current functionality of OncoSimulR is much more general, allowing it to
specify all models in, say, FFPopSim \cite{Zanini2012}, and some that, to
our knowledge (e.g., order effects) are not available from any genetics
simulator.
...
\begin{enumerate}
\item Specify the fitness effects. restrictions in the order of mutations:
  sections \ref{sepcfit} and \ref{litex}.
\item Simulate cancer progression: section \ref{simul}. You can simulate
  ...
  \begin{itemize}
  \item Decide on a model (e.g., Bozic or McFarland). This basically
    amounts to choosing a model with exponential growth (``Exp'' or
    ``Bozic'') or a model with gompertz-like growth (``McFL''). If
    exponential growth, you can choose whether the the effects of
    mutations operate on the death rate (``Bozic'') or the birth rate
    (``Exp'')\footnote{It is of course possible to do this with the
      gompertz-like models, but there is little reason to do
      it. \cite{McFarland2013} discuss this has little effect on their
...
Beerenwinkel2007,Datta2013}) multiplicative model: fitness is $\prod (1 +
s_i)$ where $s_i$ is the fitness effect of gene (or gene interaction)
$i$.  In all models except Bozic, this fitness refers to the growth rate
(the death rate being fixed to 1\footnote{You can change this if you
  really want to, but I cannot see a reason to do it.}). The original
model of \cite{McFarland2013} has a slightly different parameterization,
but you can go easily from one to the other (see also section \ref{mcfl}).

For the Bozic model, however, the birth rate is set to 1, and the death
rate then becomes $\prod (1 - s_i)$.

\subsubsection{No viability and types of models}\label{noviab}
...
$s_A = -1$, if that event happens fitness becomes immediately zero, and
that clone does not even get recorded or created as it is not viable.

Models based on Boziz, however, have a birth rate of 1\footnote{In the C++
  there is a different model, not directly callable from R for now, called
  ``bozic2'' that is slightly different. These comments apply to the model
  that is right now callable from R} and mutations affect the death
rate. In this case, a death rate larger than birth rate, per se, does not
signal immediate extinction and, moreover, even for death rates that are a
few times larger than birth rates, the clone could mutate before becoming
extinct\footnote{We said ``a few times''. For a clone of population size 1
  ---which is the size at which all clones start from mutation---, if
  death rate is, say, 50 but birth rate is 1, the probability of mutating
  before becoming extinct is very, very close to zero for all reasonable
  values of mutation rate}. How do we signal immediate extinction or no
viability in this case? You can set the value of $s = -\infty$. %% Setting a value of
%% $s < -90$ has the same effect.

In general, though, if you want to identify some mutations or some
combinations of mutations as leading to immediate extinction, no
viability, of the affected clone, set it to $-\infty$. Most examples below
evaluate fitness by its effects on the birth rate. You can see one where
we do it both ways in Section \ref{fit-neg-pos}.
...
accumulate. The meaning of these graphs, however, differs. In Figure 1 of
\cite{Korsunsky2014} we are shown a simple diagram that illustrates the
three basic different meanings of convergent arrows in these DAGs using
two parental nodes. 

We will illustrate it here with three. Suppose we focus on node ``g'' the
following figure (we will create it shortly)
<<>>=
...
\item In relationships of the type used in \textbf{Conjunctive} Bayesian
  ...
  \cite{Korsunsky2014} or \textbf{monotone} relationship in
...
assume, such as the Poisson waiting time in CBNs. We will not discuss them
here.}.


To have a flexible way of specifying all of these restrictions, we will
want, therefore to be able to say of what kind of dependency each child
...
\cite{DiazUriarte2015} it is simple to map them to a simple evolutionary
...
\cite{DiazUriarte2015}). This we can do by being explicit about the
fitness effects of these deviations from the restrictions encoded in the
DAG. (See also \ref{numfit} for the details about the meaning of the
fitness effects).
...
<<>>=
library(igraph) ## to make the layout available
...
gcbn2[56, "Fitness"] == 1.03 * 1.04 * 1.2 * 0.10
@ 
...
in, say, CBNs or OT can be considered to apply not to genes, but to
...
Note that we need to add an epistasis term (see all the details in section
\ref{epi}) to capture the idea of ``mutating both ``a1'' and ``a2''
...
\section{Order effects} \label{oe}
...
\subsubsection{Order effects and modules}\label{oemod}
...
Synthetic viability and synthetic lethality (e.g., \cite{Ashworth2011,
  Hartmann2001}) are just special cases of epistasis (section \ref{epi})
...
\ref{fit-neg-pos-sim}) in this particular case because there are ``-1'',
...
no clone viability, when we used birth rates).
...
to fitness effects. See also section \ref{theminus} and \ref{e3} and the
...
<<>>=
b1 <- evalAllGenotypes(fbauer, order = FALSE)
@ 

Order makes no difference

<<>>=
b2 <- evalAllGenotypes(fbauer, order = TRUE, max = 2000)
@ 
...
Figure 1 of \cite{Misra2014} presents three scenarios which are different
types of epistasis.
...
In \cite{Ochs2015} we find the a model that we can represent as (the
...
\subsection{McFarland parameterization}\label{mcfl}
...
$\frac{1 + s}^d/(1 - s_p)^p$. However, we can map from this ratio to the
usual product of terms by using a different value of $s_p$, that we will
call $s_{pp} = -s_p/(1 + s_p)$ (see \cite{McFarland2014-phd}, his eq. 2.1
in p.\ 9). 


Note that in v.1 we use the same parameterization as in
\cite{McFarland2013}. This reparameterization applies to v.2.
...
The next is too slow and too big to run in a vignette, because we keep
track of over 4000 different clones (which leads to a result object of
over 800 MB):
...
mcfLs <-  oncoSimulIndiv(mcfL,
                         ...
                         finalTime = 2000,
                         onlyCancer = FALSE)
plot(mcfLs, addtot = TRUE, lwdClone = 0.4, log = "")
summary(mcfLs)
...
section \label{cbn1} with two different models:
<<>>=
data(examplesFitnessEffects)
...
\subsection{Simulation with order effects and some clonal competition}
...
(We use a somewhat large mutation rate, so that the simulation runs quickly.)
...
tmp <-  oncoSimulIndiv(examplesFitnessEffects[["o3"]],
                       ...
                       extraTime = 1000)
tmp
plot(tmp, addtot = TRUE, lwdClone = 0.4, log = "")
...
point of detection. Instead of specifying \Rcode{extraTime} we can set the
\Rcode{detectionDrivers} value to a number larger than the number of
existing possible drivers, and the simulation will run until
\Rcode{finalTime} if \Rcode{onlyCancer = FALSE}.)
...
Of course, there is no trouble in using the above with other models:
...
that different individuals will be sample at the same time unit, only that
...
not identical to the ``unifor'' sampling of  \Rfunction{oncoSimulSample},
as we are not sampling uniformly over all time periods, but are stopping
at uniformly distributed values of the stopping conditions. Arguably,
however, the procedure in \Rfunction{samplePop} might be closer by what we
...
c'' and ``c > d'' both give you a genotype with ``c'' and ``d'' mutated.
...
set \texttt{removeDirectIndirect = TRUE}? Except for zz \cite{Farahani},
