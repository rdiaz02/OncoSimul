...
\bioctitle[\textit{OncoSimulR: genetic simulation with arbitrary
  ...
  populations with arbitrary epistatic interactions and focus on modeling
  tumor progression.}
...
models. Moreover, you could add passenger mutations to the
simulations. 


Since then, OncoSimulR has been vastly extended to allow you to specify,
also, other types of restriction in the accumulation of genes, as
specified by the ``semimonotone'' model of Farahani and Lagergren
...
``restrictions in the accumulation of mutations''. In this phenomenon,
...
generally, OncoSimulR also allows now to specify arbitrary epistatic
...
over sets of genes. In addition, arbitrary numbers of genes without
...
\item Simulate cancer progression: section \ref{simul}. You can simulate
  ...
  \item Decide on a model. This basically amounts to choosing a model with
    ...
    (``Bozic'') or the birth rate (``Exp'')\footnote{It is of course
      possible to do this with the gompertz-like models, but there is
      little reason to do it. \cite{McFarland2013} discuss this has little
      effect on their results, for example. In addition, decreasing the
      death rate will more easily lead to numerical problems as shown in
      section \ref{ex-0-death}}.
...
this only makes sense for subsets of the genes or for very small genotypes,
...
growth rate (the death rate being fixed to 1\footnote{You can change this
  if you really want to, but I cannot see a reason to do it.}). The
...
$\frac{(1 + s)^d}{(1 - s_p)^p}$. However, we can map from this ratio to
the usual product of terms by using a different value of $s_p$, that we
will call $s_{pp} = -s_p/(1 + s_p)$ (see \cite{McFarland2014-phd}, his
eq. 2.1 in p.\ 9).  This reparameterization applies to v.2. In v.1 we use
the same parameterization as in the original one in McFarland
\cite{McFarland2013}.
...
immediately without any chance for mutation\footnote{This is a shortcut
  ...
  alternative implementation makes a clone immediately extinct only if any
...
be specified as modules. All elements of the epistasis, posets (the DAGs)
and order effects components should be specified in terms of modules. But
you can, of course, specify a module as containing a single gene (and a
single gene with the same name as the module).
...
effect is satisfied when gene $x$ is mutated before gene $y$.
...
A simple example of synthetic viability is shown in the
following table:
