...
\section{Specifying fitness effects: some examples from the literature}
...
We will simulat
<<>>=
load("examplesFitnessEffects.RData") ## FIXME remove later, when as data.
...
tmp <-  oncoSimulIndiv(fE = examplesFitnessEffects[["o3"]],
                       model = "McFL", 
                       mu = 5e-5,
                       detectionSize = 1e8, 
                       detectionDrivers = 3,
...
\subsection{Numerical issues with Bozic}
...
ie3_b <- oncoSimulIndiv(fE = ie3, model = "Bozic")

evalAllGenotypes(ie3, order = FALSE, addwt = TRUE)

ie3_e <- oncoSimulIndiv(fE = ie3, model = "Exp")

@ 


\section{Generating DAGs}\label{simo}
...
For simplicity, most of the fitness effects specifications have been
stored in file \Robject{exampleFitnessEffects.RData}.
