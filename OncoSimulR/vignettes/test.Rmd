...
FALSE`.  The results are shown in Table \@ref(tab:timings3) (note that the
...
Table: (\#tab:timing3xf) Benchmarks of all models in Tables \@ref(tab:bench1), \@ref(tab:bench1b) and \@ref(tab:timing3) using $s=0.05$ (instead of $s=0.1$).
...
pander(benchmark_1_0.05[ , c("Elapsed Time, average per simulation (s)",
				  ...
				  "Total Population Size, mean",
 				  ...
 				  "keepEvery",
				  "PDBaseline", "n2", "onlyCancer")],
 ## caption = "\\label{tab:timing3xf}Benchmarks of all models in Tables \\@ref(tab:bench1), \\@ref(tab:bench1b) and \\@ref(tab:timing3) using $s=0.05$ (instead of $s=0.1$)."
				  )	
```
