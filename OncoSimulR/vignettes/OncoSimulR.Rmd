...
|&nbsp; Entities Tracked | Individuals (genetic clones ---\@ref(meaningclone))|
...
- sampling the data and adding different levels of noise to the
sampled data;
...
### Epistasis and predictability in rugged fitness lanscapes in asexual populations {#ex-ochs}

These questions encompass a wide range of specific issues that have
been in evolutionary genetics studies that include from detailed
...
`oncoSimulPop` ---see more details in section \@(fixation) where we
also use this same model). We would then examine the proportion of
genotypes fixed under different scenarios. For instance, we can use
the example from @Ochs2015 (we will a see this example also in
section \@(ochsdesai), where we cover different ways of specifying
fitness). I will set fitness to some specific, arbitrary values. And
we can extend this example by adding mutator genes:
...
## Set the intermediate genotype en route to ui as mildly deleterious
## so there is a valley. As in Ochs and Desai, the ui and uv genotypes
## can never appear
...
## For the sake of extending this, turn i into a mutator gene too
...
For the second set of questions, we would run simulations under
random fitness landscapes with varied ruggedness, and would then
examine the evolutionary predictability of the trajectories with
measures such as ``Lines of Descent'' and ``Path of the Maximum''
@szendro_predictability_2013 and the diversity of the sampled
genotypes under different sampling regimes.
...
## c and sd (which affect how rugged the landscape is), or reference genotype
## or evolutionary model, or stopping criterion, sampling procedure, or ...

## 1. Generate a random fitness landscape, from the Rough
##    Mount Fuji model, with g genes, and c (additivity constant) and
...
## and that requires either calling the web app or
...
develop under different variations on their model and different
...
@tomlinson_mutation_1996] and in evolutionary genetics, and are
related to wider issues such as Muller's ratchet and the evolution
of sex [@gerrish_complete_2007]. There are, thus, a large range of
questions related to mutator and antimutator genes. 


One question addressed in @tomlinson_mutation_1996, concerns under
what circumstances mutator genes are likely to play a role in cancer
progression. For instance, @tomlinson_mutation_1996 find that an
increased mutation rate is more likely to matter if the number of
required mutations in driver genes needed to reach cancer is large
and if the mutator effect is large.
...
also [@gerrish_complete_2007, @McFarland2014]).
...
exception if you do not think about the mutator effects you are
using: see \@{tomlin-except}.)
...
OncoSimulR is designed for complex fitness specifications and
selection scenarios and uses forward-time simulations; the types of
questions where OncoSimulR can be of help are discussed in sections
\@ref(generalwhatfor) and \@ref(whatfor) and running time and space
consumption of OncoSimulR are addressed in section
\@ref(timings). You should be aware that **coalescent simulations**,
sometimes also called backward-time simulations, are much more
efficient for simulating neutral data as well as some special
selection scenarios [@Yuan2012,@Carvajal-Rodriguez2010,@Hoban2011].
In addition, since OncoSimulR allows you to specify fitness with
arbitrary epistatic and order effects, you need to learn the syntax
of how to specify those effects and you might be paying a
performance penalty if your scenario does not require this
complexity. For instance, in the models in @Beerenwinkel2007b or
@Bozic2010, the fitness of a genotype depends only on the total
number of drivers mutated, but not on which drivers are mutated (and
thus, not on the epistatic interactions nor the order of
accumulation of the drivers). This means that the syntax for
specifying those fitness models could probably be a lot simpler
(e.g., specify $s$ per driver). But it also means that the code
could probably be much faster. First, because fitness evaluation is
...
as different clones if the differ anywhere in their genotype), but
only of clone types as defined by the number of mutated drivers, and
keeping track of clones can be expensive ---see sections
\@ref(timings) and \@ref(trackindivs). So for those cases where you
do not need the full flexibility of OncoSimulR, special purpose
software might be easier to use and faster to run. Of course, for
some types of problems this special purpose software might not be
available, though.
...
loaded. We also explicitly load `r Biocpkg("graph")` and `r
CRANpkg("igraph")` for the vignette to work (you do not need that
...
<!-- %                      onlyCancer = FALSE)
-->
```{r iep2x2, fig.width=6.5, fig.height=5, fig.cap= "Plot of genotypes of an epistais simulation."}
...
<!-- How long simulations take and how depend on several factors. This section -->
<!-- gives a quick view. Longer examples and more detailed comments are -->
<!-- provided in section \@ref(moretimings). -->
...
Size of return objects will depend on:
...
genes on population sizes up to $10^6$ to $10^7$.


Note that most of the code in this section is not executed during
the building of the vignette, to keept vignette build time
reasonable and prevent using huge amounts of RAM. All of the code,
ready to be sourced and run is available from the 'inst/miscell'
directory (and the summary output from some of the benchmarks is
available from the 'miscell-files/vignette_bench_Rout' directory of
the main OncoSimul repository at
...
Nindiv <- 100 ## Increase to decrease sampling variation
...
of clones, iterations, and final times doing:
...
<!-- ---------------------------------------------------------------------------------------------------------------------------------------------------- -->
<!-- Simulation    Elapsed Time (s)         Object Size (MB)   Median Number Clones  Median Number Iteration   Median Final Time  Median Total Pop. Size  -->
<!-- ----------  ---------------------    --------------------   -->
<!-- ep1_exp                  0.8              1.6 -->
...
<!--                                                 exp1      mc1      exp2       mc2	 -->
<!-- ------------------------------------------     --------- -------- ---------  -------- -->
<!-- Elapsed Time, average per simulation (s)       0.002      0.70     0.0015     0.68	 -->
											                                         
<!-- Object Size, average per simulation (MB)       0.03      4.0       0.01       0.01		 -->
											                                         
<!-- Median Number of Clones                         2         13        1          1		 -->
											                                         
<!-- Median Number of Iterations                     274       850000    253        830000 -->
											                                         
<!-- Median Final Time                               273       21700     252        20236	 -->
											                                         
<!-- Median Total Population Size                    1147      683       1057       692 	 -->
<!-- -----------------------------------------      --------- -------- ---------  -------- -->
<!-- Table: (\#tab:timing1) Benchmarks of Exp and McFL models using the default -->
...
The above table shows that a naïve comparison (looking simply at
...
the McFL model. This table also shows that the `keepEvery = NA`
setting, which was in effect in simulations "exp2" and "mc2", can
make a difference especially for the McFL models, as seen by the
median number of clones and the size of the object; we will return
to this issue below as execution time and object size (which affects
RAM usage) depend strongly on number of clones tracked.

<!-- Models `exp2` and `mc2` do not store any intermediate population samples -->
<!-- (`keepEvery = NA`) so clones that become extinct at any sampling period -->
<!-- are pruned and only the existing clones at the end of the simulation are -->
...
<!-- eventually become extinct. This makes a difference especially for the McFL -->
<!-- models, as seen by the median number of clones and the size of the -->
<!-- object. We will return to this issue below as execution time and object -->
<!-- size (which affects RAM usage) depend strongly on number of clones tracked. -->



We can run the exponential model again modifying the arguments of the
`detectionProb` mechanism; in two of the models below (`exp3` and `exp4`)
no detection takes place unless populations are at least 100 times larger
than the initial population size and probability of detection is 0.1 with
a poulation size 1000 times larger than the initial one (`PDBaseline =
5e4`, `n2 = 5e5`). In the other two models, no detection takes place
unless populations are at least 1000 times larger than the initial
population size and probability of detection is 0.1 with a poulation size
100000 times larger than the initial one (`PDBaseline = 5e5`, `n2 =
5e7`). In runs `exp3` and `exp5` we set `keepEvery = 1` and in runs
`exp4` and `exp6` we set `keepEvery = NA`.
...
<!--                                                exp3      exp4     exp5       exp6     -->
<!-- ------------------------------------------    --------- -------- ---------  --------  -->
<!-- Elapsed Time, average per simulation (s)      0.010      0.009     0.85     0.85 -->

<!-- Object Size, average per simulation (MB)      0.41       0.01      0.90       0.02 -->

<!-- Median Number of Clones                        14        9        31.5       26.51 -->

<!-- Median Number of Iterations                    2780      2770     43900      58230 -->

<!-- Median Final Time                              1870      1870     2235       1586 -->

<!-- Median Total Population Size                   8.0e6     7.8e6     1.1e9      1.3e9  -->
<!-- -----------------------------------------     --------- -------- ---------  -------- -->
<!-- Table: (\#tab:timing2) Benchmarks of Exp models modifying the default `detectionProb` with two settings of `keepEvery`. -->
...
As above, a `keepEvery = NA` (in `exp4` and `exp6`) leads to much
...
iterations (in this case by factors of about 15x to 20x) and a
corresponding increase in execution time.


Incidentally, in some of the runs of `exp5` and `exp6` we get the
(recoverable) exception message from the C++ code `Recoverable
exception ti set to DBL_MIN. Rerunning.`, which is related to those
simulations reaching total population sizes $>10^{10}$; we return to
this below (section \@ref(popgtzx)).
...
<!-- ```{r tableattemptsvalue, eval = TRUE, echo = FALSE} -->
<!-- ## From running the code -->

<!-- tableattemptsvalues <- c(exp1 = 2, mc1 = 1, exp2 = 2, mc2 = 1, -->
<!--                          exp3 = 5, exp4 = 5, exp5 = 5.5, exp6 = 5) -->
<!-- tableattemptsvalues <- data.frame(Attempts_Until_Cancer = tableattemptsvalues) -->

<!-- knitr::kable(tableattemptsvalues, booktabs = TRUE, -->
<!--              caption = "Number of attempts until cancer.", align = "c") -->
...
(The values can be obtained from any of the above runs doing, for
instance, `median(unlist(lapply(exp1, function(x) x$other$attemptsUsed)))`
).
...
<!-- There are large differences in speed between models and sizes of -->
<!-- objects differ too: in these cases, the simulations using the "McFL" -->
<!-- model run for much longer (2000 to 2000 time units, vs. 100 to 1500 -->
<!-- of the "Exp" models) and also had a larger number of clones returned -->
<!-- (5 to 16 vs. 1 to 3). As said above, for further details go to -->
<!-- section \@ref(moretimings). -->


We can rerun all of the above models with argument `onlyCancer = FALSE`
and the results are shown below (note that the differences between this
table and table \@ref(tab:bench1) for the McFL models only reflecting
sampling variation).


<!-- --------------------------------------------------------------------------------------------------------------------------------- -->
<!--                                               exp1      mc1      exp2       mc2	      exp3      exp4          exp5         exp6     -->
<!-- ------------------------------------------   --------- -------- ---------  --------  --------- ---------- -----------  -----------  -->
<!-- Elapsed Time, average per simulation (s)     0.001      0.64      0.001       0.64	  0.002     0.002      0.040         0.090 -->
											                                       
<!-- Object Size, average per simulation (MB)     0.04       3.5        0.01       0.01	   0.17     0.01        0.16         0.01 -->
											                                       
<!-- Median Number of Clones                       2         12        0          1		   2         0          2            0 -->
											                                       
<!-- Median Number of Iterations                   374       796000    312        739000    791       759        499           692 -->
											                                       
<!-- Median Final Time                             373       19940     312        18500	   7900      666        498           674 -->
											                                       
<!-- Median Total Population Size                  0         689       0          691 	   0         0          0             0  -->

<!-- Mean  Total Population  Size                  517       705       639        699       1.5e6    6.2e6       1.9e8       5.3e8 -->
<!-- -----------------------------------------    --------- -------- ---------  --------  --------- ---------- ----------   ----------- -->
<!-- Table: (\#tab:timing3) Benchmarks of models in Table \@ref(tab:timing1) and -->
<!-- \@ref(tab:timing2) when run with `onlyCancer = FALSE`. -->
...
seen by the median population size of 0 (but not all, as the mean
and max. population size is clearly away from zero). Consequently, simulations
under the exponential model are now faster and produce much smaller
objects. Of course, whether one should run simulations with `onlyCancer =
TRUE` or `onlyCancer = FALSE` will depend on the question being asked (see
\@ref(exbauer) for a question where we will naturally want to use
`onlyCancer = FALSE`).
...
(recall we are dealing with exponential growth). 
...
exit conditions will be much larger. 
...
Let us execute some simulations under more usual conditions. The
script is provided as 'benchmark_2.R', under '/inst/miscell', with
output in the 'miscell-files/vignette_bench_Rout' directory of the
main OncoSimul repository at
https://github.com/rdiaz02/OncoSimul. The data are available as
`data(benchmark_2)`.


For fitness we will use the pancreas example, two random fitness
landscapes, and four sets of independent genes (200 to 4000 genes)
...
We will use the Exp and the McFL models, run with different
parameters. For Exp the call will be
...
For the exponential model we will stop simulations when the
populations gets $>10^6$ (simulations start from 500 cells). For the
McFarland model we will use the `detectionProb` mechanism (see
section \@ref(detectprob) for details); we could have used as
stopping mechanism `detectionSize = 2 * initSize` (which would be
basically equivalent to reaching cancer, as argued in
[@McFarland2013]); in this model we will also start from 1000, not
500 (starting from 1000 we almost always reach cancer). 

Why not use the `detectionProb` mechanism with the `Exp` models?
Because it can be hard to intuitively understand what are reasonable
settings of the parameters of the `detectionProb` mechanism for a
population that is growing exponentially, especially if different
genes have very different effects on fitness and we are using
fitness specifications that are so different (compare the fitness
landscape of six genes with the pancreas specification with the
`re_4000`). In contrast, the `detectionProb` mechanism might be
simpler to reason about in the case of a population that is growing
under a model of carrying capacity with possibly large periods of
stasis. Let us emphasize that it is not that the `detectionProb`
mechanism does not make sens with the Exp model; it is simply that
the parameters might need finer adjustment for them to make sense,
and in these benchmarks we are dealing with widely different fitness
specifications.


Note that we specify `checkSizePEvery = 4` (instead of the default,
which is 20). Why? Because the fitness specifications where fitness
effects are drawn from exponential distributions (`re_200` to
`re_4000` above) include many genes (well, up to 4000) some of them
with possibly with very large effects. In this conditions,
simulations will run very fast in the sense of "units of time", so
checking every 20 units makes little sense as the population could
have increased size several orders of magnitude in between checks
(this is also mentioned in section \@ref(detectprob) and
\@ref(bench1xf)). To ensure, however, that populations have really
grown, we have increased the setting of `PDBaseline` (no simulation
can be considered for stopping unless its size is 1.4 times larger
than `initSize`).

In all cases we use `keepEvery = 1` and `keepPhylog = TRUE`.

Finally, we run all models with `errorHitWallTime = FALSE` and
`errorHitMaxTries = FALSE` sot that we can see results even if
stopping conditions are not met.
...
Results, using `r unique(benchmark_2$Numindiv)` simulations, are
shown below.
...
knitr::kable(benchmark_2[, c("time_per_simul",
    "size_mb_per_simul", "NumClones.Median", "NumIter.Median",
	"FinalTime.Median", "TotalPopSize.Median", "TotalPopSize.Mean", 
	"TotalPopSize.Max.", "Model", "fitness")], 
    booktabs = TRUE,
	col.names = c("Elapsed Time, average per simulation (s)",
	              "Object Size, average per simulation (MB)",
				  "Number of Clones, median",
...
In most cases, simulations run reasonably fast (under 0.1 seconds)
and the return objects are most of the time small. In will only
focus on a few cases.

The McFL model with the random fitness landscape `rf12` and with
`pancr` does not satisfy the conditions of `detectionProb` in most
cases: its median final time is 5000, which was the maximum final
time specified. This suggests that the fitness landscape is such
that it is extremely unlikely that we will get population sizes $>
1400$ (remember we increase the setting for `PDBaseline`). There is
nothing particular about using a fitness landscape of 12 genes or
about the pancreas DAG, and this is just a chance event: other runs
do not always show this pattern. However, complex fitness landscapes
might be such that genotypes of high fitness (those that allow
reaching a large population size quickly) are not easily accessible
so reaching them might take a long time. This does not affect the
exponential model in the same way because, well, because there is
exponential growth in that model. You might want to play with the
script and modify the call to `rfitness` (using different values of
`reference` and `c`, for instance, to have simpler paths to a
maximum) or the call to `oncoSimulPop` (with, say, `finalTime` to
much larger values). Some of these issues are related to more
general questions about fitness landscapes and accessibility (see
section \@ref(ex-ochs) and references therein).

You could also set `onlyCancer = TRUE`. This might make sense if you
are interested in only seeing simulations that "reach cancer" (or
reach a state you define as a function of size or drivers). However,
if you are exploring fitness landscapes that setting might not
always be reasonable (as reaching a particular minimal size, for
instance, might just not be possible under some fitness landscapes).



As we anticipated above, the `detectionProb` mechanism has to be
used with care: some of the simulations run in very short "time
units", such as those for the fitness specifications with 2000 and
4000 genes. Having used a `checkSizePEvery = 20` there would not
have made sense.
...
`re_500`). These are the cases with the larges number of
...
have birth rates $>1$ and, thus, clones that are unlikely to go
extinct. 




### Common use cases, set 2. {#common1}

We will now rerun the above changing the following:

- `finalTime` is now set to 25000.
- `onlyCancer` is not set to TRUE.
...
The data are available as `data(benchmark_2)`.
...
knitr::kable(benchmark_3[, c("time_per_simul",
    "size_mb_per_simul", "NumClones.Median", "NumIter.Median",
	"FinalTime.Median", "TotalPopSize.Median", "TotalPopSize.Mean", 
	"TotalPopSize.Max.", "Model", "fitness")], 
    booktabs = TRUE,
...
Now, since we increased the maximum final time and force runs to
"reach cancer" the McFL run with the pancreas fitness specification
takes a bit longer because it also has to do a larger number of
iterations. Interestingly, notice that the median final time is
close to 10000, so the runs above with maximum final time of 5000
would have had a hard time being completed.

Forcing simulations to "reach cancer", and just random differences
between the random fitness landscape also affect strongly the McFL
run under `rf12`: final time is below 5000 and the median number of
iterations is about half of what was above.  

Finally, by stopping the Exp simulations at $10^5$ simulations with
`re_2000` and `re_4000` finish now in much shorter times (but they
still take longer than their McFL counterparts) and the number of
clones created is much smaller.
...
memory usage ot running time. We discuss several examples next, that
illustrate some of the major issues to consider. Another example
with 50,000 genes is shown in section \@ref(mcf50070).

We have seen in \@ref(bench1) that for the Exp model, benchmark
results using `detectionProb` require a lot of care and can be
misleading. Here, we will fix initial and final population
sizes. Moreover, to avoid the confounding factor of the `onlyCancer
= TRUE` argument, we will set it to FALSE, so we measure directly
the time of individual runs.


### Exponential model with 10,000 and 50,000 genes {#exp50000}

#### Exponential, 10,000 genes, example 1  {#exp10000_1}
...
Each simulation takes about 1 second but note that the number of
clones for most simulations is already over 4000 and that the size
of the return object is close to 1 GB. 


#### Interlude: where is that 1 GB coming from? {#wheresizefrom}

In section \@ref(exp10000_1) we have seen an apparently innocuous
simulation producing a return object of almost 1 GB. Where is that
coming from? It means that each simulation produced about 200 MB of
output. 
...
The above shows the reason: the `Genotypes` matrix is a 10000 by
3887 integer matrix (with a 0 and 1 indicating not-mutated/mutated
for each gene in each genotype) and in R integers use 4 bytes
each. And the `pops.by.time` is 346 by 3888 (the 1 in $346 = 345 +
1$ comes from time 0 and the 1 in $3888 = 3887 + 1$ is due to the
column with the time index) double matrix and doubles use 8 bytes.
...
We can decrease the size of the return object if we use the
`keepEvery = NA` argument, as explained in detail in \@ref(bench1).
...
#### Exponential,  50,000 genes, example 1 {#exp50000_1}
...
What about the `mutationPropGrowth` setting? Let us rerun with
`keepEvery = NA` but with the default `mutationPropGrowth`:
...
time and the size of the object increases by almost 5 GB.
...
\@ref(mcf50070). We will show a few more examples with those many
genes but with a different fitness specification and playing around
...
number of clones being tracked.
...
but with mutation rate increase by a factor of 5:
...
and size of return object is similar to that of the second example.
But computing time has increased by a factor of about 5x and
iterations have increased by a factor of about 2x. Iterations
...
clones that are not shown here, because they are pruned as they
...
(where we have increase mutation rate) at each iteration we need to
...
Now let's use the default of `mutationPropGrowth = TRUE`:
...
Note the huge increase in computing time (related of course to the
huge increase in number of iterations) and in the size of the return
object: we have gone from having to track about 2000 clones to
tracking around 12000 clones even when we prune all clones without
descendants.



### An example with $s = 0.05$ {#largegenes005}

A script with the above runs but using $s=0.05$ instead of $s=0.1$
is available from the repository
(`miscell-files/vignette_bench_Rout/large_num_genes_0.05.Rout`). I
will single out a couple of cases here.
...
43 mutations (whereas only about 22 are needed if $s=0.1$).


Next, let us compare \@ref(mc50000ex1):
...
object is much smaller than above. 




### The different consequences of `keepEvery = NA` in Exp and McFL models {#kpexpmc}

We have seen that `keepEvery = NA` often leads to much smaller
return objects when using the McFarland model than when using the
Exp model. Why? Because in the McFarland model there is strong
competition and there can be complete clonal sweeps so that a single
clone might be all that is left after some time. This is not the
case in the exponential models. This, of course, is not always the
case and can depend on the difference in fitness effects between
different genes. In particular, we have seen several examples where
even with `keepEvery=NA` there are a lot of clones in the McFL
models because there are many clones of similar fitness so no clone
ends up displacing the others.
...
As explained in section \@ref(trackindivs) OncoSimulR prunes clones
that never had a population size larger than zero at any sampling
period (so they are not reflected in the `pops.by.time` matrix in
the output); when set `keepEvery = NA` we are telling OncoSimulR to
...
It is true that the utility of keeping the complete history is
limited if the `pops.by.time` matrix only contains the very last
period. However, you can use `plotClonePhylog`
and ask to be shown only clones that exist in the very last period.

For instance, in run \@ref(exp50000_1) we could have looked at the
information stored for the history of clones by doing:
...
$4e^{15}$ as in those cases loosing precision (as we are using
...
"/inst/miscell"). However, well before we reach $4e^{15}$ we loose
precision from other sources. One of the most noticeable ones is
that when we reach population sizes around $1e^{11}$ the C++ code
will often alert us by throwing exceptions with the message
`Recoverable exception ti set to DBL\_MIN. Rerunning.`; I throw this
...
minutes. We keep the history of the clones and use `keepEvery = 1`.
...
We can run the exponential model with `detectionSize = 1e11`:
...
Note that we almost reached the `max.wall.time` (600 * 5 =
3000). What if we wanted to go up to $10^{12}$? We could not do it
in 10 minutes. We could set `max.wall.time` to a value larger than
600 to allow us to reach larger sizes but then we would be waiting
for a possibly intolerable time for simulations to finish. Moreover,
this would eventually fail as simulations would keep hitting the
$t_i$ exception without ever being able to complete. Finally, even
if we are very patient, hitting that $t_i$ exception should make us
worry about possible biases in the samples.
...
To summarize this section, we have seen: 
  - Both McFL and Exp can be run in short times over a range of
      sizes for the `detectionProb` mechanism using a complex
      fitness specification with a small number of genes.
  ...
  - Simulations start to slow down and lead to a very large object
    size when we keep track of around 6000 to 10000 clones. Whatever
    leads to these patterns will slow down the simulations.
	
  - OncoSimulR needs to keep track of genotypes, not just numbers of
    drivers and passengers, because it allows you to use complex
    fitness and mutation specifications that depend on specific
    genotypes. The `keepEvery = NA` is an approach to keep the
    minimal history needed, but it is unavoidable that during the
    simulations we might be forced to deal with many thousands of
    different clones.
...
for stopping the simulations (see \@{fixation}).
...
We have seen many of these plots already, starting with
\@ref(fig:iep1x1) and \@ref(fig:iep2x2) and we will see many more
below, in the examples, starting with section \@ref(bauer2) such as
in figures \@ref(fig:baux1) and \@ref(fig:baux2). In a nutshell,
what we are plotting is the information contained in the
...
The functions that do the work are called `plot` and these are
actually methods for objects of class "oncosimul" and "oncosi
mulpop". You can access the help by doing `?plot.oncosimul`, for
example.
...
- genotypes --or clones or haplotypes (e.g., \@ref(fig:baux2)).
...
All those three are shown in both of \@ref(fig:baux1) and \@ref(fig:baux2).
...
fitness even a moderate `checkSizPEvery` of, say, 10, might be
...
conditions). There is a special exception to this porcedure: if you
...
## Fixation of genes/gene combinations with Ochs and Desai example {#fixation}
...
Simulations will stop as soon as any of the genes or gene
combinations in the vector (or list) `fixation` reaches a frequency
of 1.  These gene combinations might have non-zero intersection
(i.e., they might share genes), and those genes need not be
drivers. If we want simulations to only stop when fixation of those
genes/gene combinations is reached, we will set all other stopping
conditions to `NA`. It is, of course, up to you to ensure that those
stopping conditions are reasonable (that they can be reached) and to
use, or not, `finalTime`; otherwise, simulations will eventually
abort (e.g., when `max.wall.time` or `max.num.tries` are
reached). Since we are asking for fixation, the `Exp` or `Bozic`
models will often not be appropriate here; instead, models with
competition and limited population size such as `McFL` are more
appropriate.


We return here to the example from section \@(ochsdesai). 
...
explicitly to `NA`. (We set the number of repetitions only to 50 for
...
(as explained in \@ref(endsimul)). Then, you will use function
...
- `oncoSimulPop`, with argument `keepEvery = NA`. 
     ...
     simulations (as explained in \@ref(endsimul)).  Then, you will
...
Regardless of whether and how you plot the information in
`pops.by.time`, you can also sample one or multiple simulations
using `samplePop`. In **whole-tumor** sampling the resolution is the
whole tumor (or the whole population). Thus, a key argument is
`thresholdWhole`, the threshold for detecting a mutation: a gene is
considered mutated if it is altered in at least "thresholdWhole"
proportion of the cells in that simulation (at a particular time
point). This of course means that your "sampled genotype" might not
correspond to any existing genotype because we are summing over all
cells in the population; for instance suppose that at the time we
take the sample there are only two clones in the population, one
with a frequency of 0.4 that has gene A mutated, and a second one
with a frequency of 0.6 that has gene B mutated. If you set
`thresholdWhole` to values $leq 0.4$ the sampled genotype will show
both A and B mutated. **Single-cell** sampling is provided as an
option in contrast to whole-tumor sampling. Here any sampled
genotype will correspond to an existing genotype as you are sampling
with single-cell resolution.


When `samplePop` is run on a set of simulated data of, say, 100
simulated trajectories (100 "subjects"), it will produce a matrix
with 100 rows (100 "subjects"). But if it makes sense in the context
of your problem you can of course run `samplePop` repeatedly.
...
full mapping genotyeps to fitness, or by providing that mapping by
...
consequences of different types of epistatic interactions (e.g.,
@szendro_quantitative_2013, @franke_evolutionary_2011) and there are
...
followed @szendro_quantitative_2013, @franke_evolutionary_2011 and
model fitness as
...
Several measures of evolutionary predictability have been proposed
in the literature (see, e.g.,@szendro_predictability_2013 and
references therein). We provide two, Lines of Descent (LOD) and
Paths to the Maximum (POM), following @szendro_predictability_2013,
and a simple measure of diversity of the actual genotypes sampled.
...
provide the details on how the LODs are obtained. My implementation
is not exactly identical to their definition. My `lod_single` is the
first path to arrive at the genotype that eventually becomes the
most populated genotype at the final time (and, in this sense,
agrees with the LOD of Szendro et al.). However, in contrast to what
is apparently done in Szendro et al. ("A given genotype may undergo
several episodes of colonization and extinction that are stored by
the algorithm, and the last episode before the colinization of the
final state is used to construct the step."), I do not check that
this genotype (which is the one that will become the most populated
at final time) does not become extinct before the final
colonization. So there could be other paths (all in `all_paths`
list) that are actually the one(s) that are colonizers of the most
populated genotype (with no extinction before the final
colonization). I addition, and in case this might be useful, for
each simulation I keep all the paths that "(...) arrive at the most
populated genotype at the final time" (first paragraph in p. 572 of
Szendro et al.), whereas they only keep one (see second column of
p. 572). 
...
## This shows the POM of each of the 16 simulations
POM(pancr16)
...
In this vignette we often use "clone" or "genotype"
interchangeably. A clone denotes a set of cells that have identical
genotypes. So if you are using a fitness specification with four
genes (i.e., your genome has only four loci), there can be up to $16
= 2^4$ different genotypes or clones. Any two entities that differ
in the genotype are different clones. And this applies regardless of
whether or not you declare that some genes (loci) are drivers or
not. So if you have four genes, it does not matter whether only the
first or all four are regarded as drivers; you will always have at
most 16 different clones or 16 different genotypes.


Analogously, if you had 100 genes, 10 drivers and 90 passengers, you
will have up to $2^{100}$ different clones or genotypes. Sure, one
cell might have driver A mutated and passenger B mutated, and
another cell might have driver A mutated and passenger C mutated. So
if you only look at drivers you might be tempted to that they are
"the same clone"; but they really are not the same clone (as they
differ in their genotype). If you want summaries of simulations that
collapse over some genes (say, some "passengers", the 90 passengers
we just mentioned) look at the help for `samplePop`, argument
`geneNames`.  This would allow you, for instance, to look at the
diversity of clones/genotypes, considering as identical those
genotypes that only differ in genes you deem relevant; something
similar to defining a "drivers' clone" as all those sets of cells
that have identical genotype with respect to only the drivers (so
that in the example just mentioned both cells would be consider "the
same clone" as they only differ with respect to
passengers). However, this only applies to summaries of simulations
once we are done simulating data. OncoSimulR will always track
clones, as defined above, regardless of whether many of those clones
have the same genotype if you only focus on driver genes; see also
section \@ref(trackindivs).

Labeling something as a "driver", therefore, does not affect what we
mean by clone. Yes, labeling something as a driver can affect when
you stop simulations if you use `detectionDrivers` as a stopping
mechanism (see section \@ref(endsimul)). But, again, this has
nothing to do with the definition of "clone".


## Does OncoSimulR keep track of individuals or of clones? And how can you keep track of such large populations? {#trackindivs}
...
section \@ref(meaningclone)).  We are interested in examining the
effects of selection, mutation, keeping track of the parent-child
relationships between clones, etc, and thus we must keep track of
the complete set of clones.

We do not need to keep track of individual cells because, for all
purposes, and since we do not consider spatial structure, two or
more cells that are genetically identical are interchangeable. This
means, for instance, that the computational cost of keeping a
population of a single clone with 1 individual or with 10^9
individuals is exactly the same: we just keep track of the genotype
(or haplotype if you want) and the number of cells.
...
OncoSimulR does something very similar. Note that we never prune any
...
output). Note also that if you ask OncoSimulR to keep track of the
complete parent-child relationships, you might see in the genealogy
clones that are not present in `pops.by.time`, if these are clones
that never had a population size larger than 0 at any sampling
time. To give an example, suppose that clone A, with a population
...
that at time 2 only clone C is alive, and suppose we only sample at
times 1 and 2. Now, at time 1 clones B and C did not yet exist, and
clone B is never alive at times 1 or 2, but we cannot remove clone B
from our genealogy if we want to reflect the complete story of
C. Thus, `pops.by.time` will show only clones A and C (not B) but
the complete genealogy will show clones A, B, C. Since function
`plotClonePhylog` offers a lot of flexibility with respect to what
clones to show depending on their population sizes at different
times, you can prevent being shown B, but its existence is there
should you need it (see also \@ref(histlargegenes)).



Keeping track of clones (not individuals) and erasing clones that
have no descendants from the `pops.by.time` matrix (and other
internal structures in the C++ code, such as the map of genotypes to
next mutation time or the table of genotypes) therefore leads to
important memory and time savings. However, as explained in section
\@{moretimings}, as we have to keep track of clones, very large
genomes can represent a problem if they lead to the creation and
tracking of many different clones. (Here, programs that only try to
keep track of numbers of drivers, not individual clones, can of
course achieve further speed improvements).
...
SSA [SSA is the original Gillespie Stochastic Simulation Algorithm]
...
<!-- We record the largest difference between the computed -->
<!--     death rate and the death rate that would have been computed if -->
<!--     differences in population sizes had been caused by the addition -->
<!--     or subtraction of a single individual. Death rates are the -->
<!--     fitness are affected by density dependence and, thus, it is on -->
<!--     the death rates where the approximation that they are constant -->
<!--     over a short interval plays a role. If we were not using an -->
<!--     approximation, the two death rates above would be -->
<!--     identical. More precisely, let $A$ and $C$ denote two successive -->
<!--     sampling periods, with $D_A = log(1 + N_A/K)$ and $D_C= log(1 + -->
<!--     N_C/K)$ their death rates. `errorMF_size` stores the largest -->
<!--     $abs(D_C - D_A)$ between any two sampling periods. In addition, -->
<!--     we compute the largest difference had the change been larger -->
<!--     than the addition/subtraction of a single individual. Suppose -->
<!--     $N_C > N_A$; then we record $error = (D_C - log(1 + (N_A + -->
<!--     1)/K))/D_A$, where the second term corresponds to the death rate -->
<!--     that would had been computed from population size $N_A$ by -->
<!--     adding one individual. If $N_A > N_C$ we record $error = -->
<!--     (log(1 + (N_A - 1)/K) - D_C)/D_A$. The value `errorMF` -->
<!--     corresponds to the largest value of $error$ ever seen during a -->
<!--     simulation.  Additionally, a simple procedure to use is to run -->
<!--     the simulations with different values of `sampleEvery`, say the -->
<!--     default value of 0.025 and values that are 10, 20, and 50 times -->
<!--     larger or smaller, and assess their effects on the output of the -->
<!--     simulations and the `errorMF` statistic itself. You can check -->
...
## Sometimes I get exceptions when running with mutator genes {#tomlin-except}

Yes, sure, the following will cause an exception; this is similar to
the example used in \@(ex-mut-antimut) but there is one crucial
difference:
