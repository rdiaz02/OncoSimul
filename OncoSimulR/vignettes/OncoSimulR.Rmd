...
```{r iep1x1,fig.width=6.5, fig.height=4.5}
...
<!-- %                      onlyCancer = FALSE)
-->
```{r iep2x2,fig.width=6.5, fig.height=5}
...
genes have very different effects on fitness. In contrast, the
`detectionProb` mechanism might make more sense in the case of a
population that is growing under a model of carrying capacity with
possibly large periods of stasis.
...
zz: pay attention to num iterations
...
zz: genealogies and tracking history: be consistent
...
zz: why detectionProb more sense for McFL
...
below, in the examples, starting with section \@ref(buaer2) such as
in figures \@ref(fig:baux1) and \@ref(fig:buax2). In a nutshell,
...
zz: complete history as history of pop changes. Different from
genealopy. Clarify.

zz: clone is equivalent to genotype of haplotype
...
identical over the whole set of genes/markers you are using). We are
interested in examining the effects of selection, mutation, keeping
track of the parent-child relationships between clones, etc, and
...
should you need it (see also \@ref(histlargenes)).
