...
This model evaluate if each locus is either high or low fitness  (which corresponds to "e" parameter value), with a systematic change between each neighbor.
...
Ising model is also available [e.g., @ferretti_measuring_2016;
                               @brouillet_magellan:_2015] by using `model = "Ising"`.
The implementation of this model is decribedin [@brouillet_magellan:_2015],
and we use a call to MAGELLAN code to generate the landscape.

In Ising model only loci at the same configuration are "compatible".
Each loci interacts with its phisical neighbors, and for every one in
not identical configurations, a cost for incompatibility `i` is applied.


```{r isingex1}
rnk <- rfitness(g = 4, model = "Ising", i = 0.002, I = 0.45)
plot(rnk)
```
...
* Multiplicative: `s` and `S` mean and SD for generating random fitnesses. `d` 
...
All models can be taken into account for the fitness calculation.
With default parameters, neither of Ising, Eggbox or Optimum contribute to 
fitness lanadcape generation as all `i`, `e`, `o` and `p` all `== 0`.
Also, as all parameters refering to standard deviations have value `== -1`,
those are also have no effect unless changed.
Go to MAGELLAN webpage <http://wwwabi.snv.jussieu.fr/public/Magellan/> for 
detailed information [@brouillet_magellan:_2015].
