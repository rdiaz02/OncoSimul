...
stopping conditions are reasonable (that they can be reached);
otherwise, simulations will eventually abort (e.g., when
`max.wall.time` or `max.num.tries` are reached). Since we are asking
for fixation, the `Exp` or `Bozic` models will often not be
appropriate here; instead, models with competition and limited
population size such as `McFL` are more appropriate. 
