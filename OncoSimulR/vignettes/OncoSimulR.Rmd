...
Using Bauer's example, we may define another type of plot to represent the evolution of tumor population, using the package **ggmuller**, which can be downloaded from CRAN repository. This type of plot is specified by "fish" option upon calling to plot.oncosimul method. In a similar way to the code in \@ref({r prbaux001}), an oncoSimulIndiv object (b1) is created. Once the ggmuller package is already installed, the function plot_muller must be called on the b1 object, as it is depicted in \@ref{r_muller_code}, just below. 
...
plot_muller <- function(onco_simul_obj) {
  ...
  #First, the adjacency matrix is created.
  print("Adjacency matrix")
  LOD <- data.frame(onco_simul_obj$other$LOD_DF)
  levels(LOD$parent)[1] <- "wt"
  
  pob1 <- as.data.frame(onco_simul_obj$pops.by.time)
  
  onco_simul_obj$GenotypesLabels[1] <- "wt"
...
All in all, we have defined a new option for the plot of any oncosimul object. In the plot.oncosimul method shown in \@ref({method_plo_oncosimul}), another type of plot is added: **fish**, which allow us to call the plot_muller function explained above, to obtain the new type of plot, as seen with the if/else command: *if(type == "fish")*. 
