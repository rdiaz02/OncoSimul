...
* From version 2.13.2, you can use simulations with frequency-dependent fitness:
  fitness (birth rate) is not fixed for a genotype, but 
  can be a function of the frequecies of the clones. Evaluating frequently the 
  fitness across the simulation, you can obtain a dynamic fitness landscape 
  (see section \@ref(fdf)).
...
* Simulating asexual evolution (the `oncoSimul*` functions) where:
     - Fitness is:
        ...
        - A function of clone's frequencies (see section \@ref(fdf) for more details).   
     - Mutation rates can:
          - Vary between genes
...
### Simulating trayectories employing the avalalaible models with frequency dependent fitness 
...
**This functionality is only available in the freq-dep-fitness
branch** See: https://github.com/rdiaz02/OncoSimul/tree/freq-dep-fitness.
...
function of the abundance (relative or absolute) of other genotypes.

So the procedure is to first create a data frame with the mapping
between genotypes and their (frequency-dependent) fitness, similar
to section \@ref(explicitmap). For example, a two-column data frame,
where the first column are the genotypes and the second column
contains, as strings, the expressions for the function that relate
fitness to frequencies of other genotypes. (We can also use a data
frame with $g + 1$ columns; each of the first $g$ columns contains a
1 or a 0 indicating that the gene of that column is mutated or
not. Column $g+ 1$ contains the fitness specifications; see
`oncoSimulIndiv` for examples).

We use library [ExprTk](https://github.com/ArashPartow/exprtk) for
the fitness specifications so the range is functions you can use is
very large (http://www.partow.net/programming/exprtk/).

Once this data frame is created, we can pass it to
`allFitnessEffects`. From there, simulation proceeds as usual.
...
genotypes (that is why we use `f_1`, for example, to denote relative
frequency of genotype with the first gene mutated, `f_1_2` to denote
relative frequency of the genotype with the first and second genes
mutated, etc). 

*Note: except for the example of \@ref(hurlbut) (based on
@hurlbut2018), the examples below are not used because of their
biological realism, but rather to show some key features of the software*


In the example below, the birth rate of genotype "A" $= 1.2 + 1.5*
f\_1\_2$ and that of the wildtype $= 1 + 1.5* f\_1\_2$.  Genotype
"A, B", thus, could be a genotype whose presence leads to an
increase in the other genotypes (maybe via diffusible factors,
inducing angiogenesis, etc), but genotype "B" does not show
...
further details. Since we can use all of the functionality from
ExprTk, we can make fitness and arbitrarily complex function of the
...
We will use McFL model, so in addition to the frequency dependence in
...
They provide, in p. 4, the payoff matrix reproduced in Table
\@ref(fig:hurlbutpay): 


```{r hurlbutpay, out.width='6cm', eval=TRUE,echo=FALSE, fig.cap="Payoff matrix from Table 2 of Hurlbut et al., 2018, 'Game Theoretical Model of Cancer Dynamics withFour Cell Phenotypes', *Games*, 9, 61; doi:10.3390/g9030061."}
...
Table \@ref(fig:hurlbutpay).


To allow modelling scenarios with different values for the
parameters in Table \@ref(fig:hurlbutpay) we will define a function
...
We can check we recover Table \@ref(fig:hurlbutpay):
```{r hbf1check}
