...
To get a first idea, we will use the fitness specification from section
\@ref(quickexample), with the `detectionProb` stopping mechanism (see
\@ref(detectprob)).  We will use the two main growth models (exponential
and McFarland). Each model will be run with two settings of
`keepEvery`. With `keepEvery =1` (runs `exp1` and `mc1`), population
samples are stored at time intervals of 1 (even if most of the clones in
those samples later become extinct). With `keepEvery = NA` (runs `exp2`
and `mc2`) no intermediate population samples are stored, so clones that
become extinct at any sampling period are pruned and only the existing
clones at the end of the simulation are returned.
...
GNU/Linux. Most of the code in this section is not executed during
...
directory (and the summary output from the benchmarks from the
'data' directory).
...
The above table shows that a naïve comparison (looking simply at execution
...
smaller in the Exp than in the McFL model. This table also shows that the
`keepEvery = NA` setting can make a difference especially for the McFL
...
size (which affects RAM usage) depend strongly on number of clones
tracked.
...
100000 times larger than the initial one (`PDBaseline = 5e5`, `n2 = 5e7`).
...
As above, a `keepEvery = NA` leads to much smaller object sizes and
slightly smaller numbers of clones and execution times. Changing the
exiting conditions (by changing `detectionProb` arguments) leads to large
increases in number of iterations (in this case by factors of about 15x to
20x) and a corresponding increase in execution time.
...
We have seen in \@ref(bench1) that, specially for the Exp model,
benchmark results using `detectionProb` can be misleading. Here, we
will fix initial and final population sizes. Moreover, to avoid the
confounding factor of the `onlyCancer = TRUE` argument, we will set
it to FALSE, so we measure directly the time of individual runs.
