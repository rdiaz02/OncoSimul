...
|**Output**||
|&nbsp; Data Type| Genotype or Sequence, Individual Relationship (complete parent-child relationships between clones), Demographic (populations sizes of all clones at sampling times), Diversity Measures (LOD, POM, genotypes), Fitness|
...
|&nbsp; Demographic||	
|&nbsp; &nbsp; Population Size Changes|	Exponential, Logistic |
|&nbsp; Fecundity|	Individually Determined (fitness), Influenced by Environment (population size)|
|&nbsp;Natural Selection||	
|&nbsp; &nbsp; Determinant|	Single-locus, Multi-locus, Fitness of Offspring,  Environmental Factors (population size)|
...
|&nbsp; Events Allowed|	Varying Genetic Features (change of individual mutation rates)|
...
Following the above we will run two very minimal examples. First a
...
## Questions that can be addressed with OncoSimulR {#whatfor}

The examples in \@(quickexample) focus on the mechanics. Here, we
will list some general scientific questions in cancer genomics and
evolutionary genetics where OncoSimulR could be of help. This
section does not try to provide an answer to any of these questions
(those would be full papers by themselves) but simply to illustrate
where you can use OncoSimulR; the possible uses of OncoSimulR are
only limited by your ingenuity. Finally, I will only use short
snippets of working code as we are limited by time of execution; for
real work you would want many more scenarios, many more simulations,
etc.
...
s1 <- oncoSimulPop(2, allFitnessEffects(g1, drvNames = 1:4))
...
`oncoSimulPop`) and would then examine the proportion of genotypes
fixed under different scenarios. For instance, we can use the
example from @Ochs2015 (we will a see this example also in section
\@(ochsdesai), where we cover different ways of specifying
...
## Ochs and Desai explicitly say "Each simulated population was evolved until
## either the uphill genotype or valley-crossing genotype fixed." So we will
## use "fixation".
...
How long simulations take depend on several factors. This section
...
Note the very large variability in reaching fixation (we show here
the summary from only 20 simulations)
```{r sum-simul-ochs}
head(summary(od100)[, c(1:3, 8:9)], n = 20)
```
...
\@{moretiming}, as we have to keep track of clones, very large
