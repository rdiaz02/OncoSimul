...
genotypes, as in game theory and adaptive dynamics.
...
* From version 2.13.2, you can use simulations with
  frequency-dependent fitness: fitness (birth rate) is not fixed for
  a genotype, but can be a function of the frequecies of the clones
  (see section \@ref(fdf)). We can therefore use OncoSimulR to
  examine, via simulations, results from game theory and adaptive
  dynamics and study complex scenarios that are not amenable to
...
|&nbsp; Fitness Components||	
|&nbsp; &nbsp; Birth Rate|	Individually Determined from Genotype
(models "Exp", "McFL", "McFLD"). Frequency-Dependently Determined from Genotype
(models "Exp", "McFL", "McFLD")|
|&nbsp; &nbsp; Death Rate|	Individually Determined from Genotype
(model "Bozic"), Influenced by Environment ---population size
(models "McFL" and "McFLD")|
|&nbsp;Natural Selection||	
|&nbsp; &nbsp; Determinant|	Single and Multi-locus, Fitness of
Offspring,  Environmental Factors (population size, genotype frequencies)|
|&nbsp; &nbsp; Models|	Directional Selection, Multi-locus models,
Epistasis, Random Fitness Effects, Frequency-Dependent|
...
The new frequency-dependent fitness funcionality allows users to run simulations 
in a different way, defining fitness (birth rates) as functions of clone's frequencies.
We can thus model frequency-dependent selection.
See section \@ref(fdf) for further details and examples.
...
In the case of frequency-dependent fitness simulations, the fitness effects
must be reevaluated frequently so that birth rate, death rate or both,
depending the model used, are updated. To do this it is
necessary to use a short step to reevaluate fitness; this is done using a
small value for `sampleEvery` parameter in `oncoSimulindv` (see \@ref(whatgoodsampleevery) for more details), as is the case when using McFarland model.
...
With frequency-dependence fitness we can make fitness (actually,
birth rate) depend on the frequency of other genotypes.  We specify
how the fitness (birth rate) of each genotype depends (or not) on
other genotypes.  Thus, this is similar to the explicit mapping of
genotypes to fitness (see \@ref(explicitmap)), but fitness can be a
function of the abundance (relative or absolute) of other
genotypes. Frequency-dependent fitness allows you to examine models
from **game theory** and **adaptive dynamics**.

Since birth rate can be an arbitrary function of the frequencies of
other clones, you can model competition, cooperation and mutualism,
...
their own be asymmetric on their birth rates). See examples
\@ref(predprey).
...
`oncoSimulIndiv` for examples). Once this data frame is created, we
can pass it to `allFitnessEffects`. From there, simulation proceeds
as usual.

How complex can the functions that specify fitness be? We use
library [ExprTk](https://github.com/ArashPartow/exprtk) for the
fitness specifications so the range of functions you can use is
very large (http://www.partow.net/programming/exprtk/),
including of course the usual arithmetic expressions, logical
expressions (so you can model thresholds or jumps), and a wide range
of mathematical functions (so linear, non-linear, convex, concave,
...
genotypes; we use `f_1`, for example, to denote the relative
frequency of the genotype with the first gene mutated, `f_1_2` to
denote the relative frequency of the genotype with the first and
second genes mutated, etc ---below, in sections \@ref(fdfabs) and
\@ref{fdfrelabs} we will use absolute number of cells instead of
...
vignette, for example in \@ref(ex-ochs) or \@ref(bauer) or
\@ref(quickexample). Because of the way the code works, we need to
pass the populations sizes at which we want fitness evaluated in
...
(\@ref(fdfabs)) an example with absolute numbers.
...
Let us create the `allFitnessEffects` object and simulate.
We will use the McFL model, so in addition to the frequency dependence in
the birth rates, there is also  density dependence
in the death rate (see section \@ref(specfit)).
...
As explained in p. 6 (equation 1), we can write the fitness of the
four types as **f**=G**u**, where **f** and **u** are the vectors
with the four fitnesses and frequencies (where each element of **u**
corresponds to the relative frequencies of stromal, angiogenic,
proliferativa, and cytotoxic cells), and G is the payoff matrix in
Figure \@ref(fig:hurlbutpay).
...
to create the data frame of frequency-dependent fitness. First, we
...
To show two examples, we will run the analyses they use for figures
3a and 3b (p.8 of their paper):
...
Of course, since you can always use an absolute specification with
...
Since birth rate can be made arbitrary functions of frequencies of
other clones, we can easily model classical ecological models, such
as predator-prey, commensalism and, more generally,
...
Lotka-Volterra equations. To model it, we use the "Exp" model, which
has a constant death rate of 1, and directly translate the usual
expressions (e.g., expressions 3.15a and 3.15b in p. 73 of
@Otto2017) for rates of change into the birth rate.  Of course, you
...
Since the WT genotype is just a trick used to get the system going,
we will make sure it disappears from the system soon after we get it
to have both preys and predators. Finally, note that we can make
this much more sophisticated; for example, we could get the system
going differently by having different mutations from WT to prey and
predator. 


### Predator-prey {#predprey1}
First, create a function that will generate the usual Lotka-Volterra
expressions for consumer-resource models: 
...
Now, run a predator-prey model by setting appropriate parameters;
the key is that $a_12 > 0$ and $a_21 < 0$.
...
You can easily play with a range of parameters, say the carrying
capacity of predators and prey, to see how they affect the
...
### Commensalims {#commens}
Modelling commensalim simply requires changing the values of the
$\alpha$, the `a_12` and `a_21`. This could do it (not run, as this
is just repetitive):
...
## Can use the BNB algorithm to model frequency-dependent fitness? {#bnbfdf}
