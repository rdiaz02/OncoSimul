...
\@ref(generalwhatfor) and \@ref(whatfor) and time and space
requirements of OncoSimulR are addressed in section
...
easier. Second, and possibly much more important, because what
we need to keep track of leads to much simpler and economic
structures: we do not need to keep track of clones as types of cells
with different mutations, but only of clone types as defined by the
number of mutated drivers, and keeping track of clones can be
expensive ---see section \@ref(timings). So for those cases where
you do not need the full flexibility of OncoSimulR, special purpose
...
# Time and space requirements of OncoSimulR {#timings}

Time to complete the simulations and size of return objects depend
on several, interacting factors. The usual rule of "experiment
before launching a large number of simulations" applies, but here we
will walk through several cases to get a feeling for the major
factors that affect speed and size. Many of the comments on this
section need to use ideas discussed in other places of this
document. If you read this section first, you might want to come
back after reading the relevant parts.
...
In the sections that follow, we go over several cases to illustrate
some of the main settings that affect execution time and size of
returned objects. It should be understood, however, that many of the
examples shown below do not represent typical use cases of
OncoSimulR, but are used here to understand what, and how, affects
time and size. As we will see in most examples in this vignette,
...
available from the 'data' directory as data set zz).  zz: specify which



zz: explain keepPhylog does not add much time (some space), but a moot
point if keepEvery = NA.
...
Will run `r unique(benchmark_1$Numindiv)` simulations. <!-- This -->
<!-- represents a small number of simulations in a particular epistasis -->
<!-- scenario we are interested in. --> The results I show are for a laptop
with an 8-core Intel Xeon E3-1505M CPU, running Debian
GNU/Linux. 



zz: improve output: make the key settings clear: keepEvery and
detectionProb settings.

<!-- zz: fixme!!! explain sv2  -->
...
memory or CPU usage. We discuss several examples next, that
...
zz: not the typical use cases see also section where I discuss all this.
...
Examples above: no setting for finalTime, but a moot point (all well
< 1000)

Other factors: initial size


Illustrates that OncoSimulR pays a price if we want to track
history. Also we need to keep genotypes, not just numbers of drivers
and passengers: this is needed for complex fitness models with complex
epistasis and order effects.

In general, more than 10000 clones a huge slowdown and increase in
memory consumption.

zz: why prunning rarely matters with exp but does with McFL. But it
might matter with exp depending on fitness details.

zz: pay attention to num iterations



McF and Exp: often the case is that Exp uses a lot fewer iterations

### Are we keeping the complete history of the clones? {#histlargegenes}
...
What is the cost of tracking the history? In terms of time it is
minor. In terms of space, and as shown in the example above, we can
end up storing a data frame with tends of thousands of rows and
three columns, two of them factors. In the example above the size of
that data frame is approximately 2 MB for a single simulation. This
can quickly build up if you routinely launch, say, 1000 simulations
via `oncoSimulPop` and that is why the default is `keepPhylog =
FALSE`.
...
start all simulations from a large population (to make it very
unlikely that the population will become extinct before a cell
mutates). We set the maximum running time to 10 minutes. We keep the
history of the clones and use `keepEvery = 1`.
...
Notice the exception with the warning about $t_i$. Notice also that
this takes a long time and we run a very large number of iterations.


We can run the exponential model with `detectionSize = 1e10`:
...
summary(exp_k_50_1e10)[, c(1:3, 8, 9)]
##   NumClones TotalPopSize LargestClone FinalTime NumIter
...
However, with those same settings, we can not reach $10^{11}$ in 10
minutes. We could set `max.wall.time` to a value larger than 600 to
allow us to reach larger sizes but then we would be waiting for a
possibly intolerable time for simulations to finish. Moreover, this
would eventually fail as simulations would keep hitting the $t_i$
exception without ever being able to complete. Finally, even if we
are very patient, hitting that $t_i$ exception should make us worry
about possible biases in the samples.
...
history of the population, with a granularity is controlled by
...
variables obtained under this model. In this random fitness
landscape the fitness of the genotypes is a random variable
