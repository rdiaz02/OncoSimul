...
# Time and speed requirements of OncoSimulR? {#timings}
...
* Any factor that affects the number of clones tracked/returned, in
  ...
  and how often you keep data (the `keepEvery` argument).
...
Yes. However, large numbers of genes can lead to unacceptable memory
or CPU usage. We discuss several examples next, that illustrate some
of the major issues to consider. Another example with 50,000 genes
is shown in section \@ref(mcf50070).
...
## Population sizes larger than $10^{10}$ {#popgtzx}

McFarland: easily pop sizes of 1e6 or 1e7. For Exp, 1e7 to
1e9. Starting at 1e10 we run into trouble

Yes but:
anything that affects number of clones will huge times and object
sizes blablab
Longer genomes, larger mu even worse with larger pop sizes. But
large pop sizes bring other issues of their own.

I will use much smaller genomes to focus on the problems inherent to
large pop. sizes:


Focus on the "examples-large.R script

So with and without keepEvery = NA (or 1)


We have already seen examples where population sizes are larger than
$10^{8}$, as in Tables \@ref(tab:bench1b), \@ref(tab:timing3),
\@ref(tab:timing3xf). 
...
## Number of genes and population size {#bench2}
...
Since we want to examine the effects of number of genes, our fitness
specification will be a simple one as follows:

```{r fitspecbench2, echo = TRUE, eval = FALSE}
allFitnessEffects(noIntGenes = c(rep(0.1, ng/2), rep(-0.1, ng/2)))

```
so that for a number of genes $ng$, half of the genes have a
positive fitness effect and the other half a negative effect.
...
<!-- Uncomment when ready. O.w. confusing. -->
<!-- # Additional comments and examples about time, size, and simulation limits {#moretiming} -->

<!-- Place here the example from mcfl as slow because of the size of allFitnessEffects -->
<!-- Trick in McFl is possibly just keeping track of the number of which -->
<!-- is a lot simpler than of the full clones. -->

<!-- Add also comments on where it is not reasonable to use OncoSimulR -->
<!-- many genotypes ---we keep track of them--- -->
<!-- only care about number of drivers or passengers --we keep track of -->
<!-- genotypes -->
<!-- need fixed population sizes (strict FisherWrifht or Moran) -->



# Additional comments and examples about time, size, and simulation limits {#moretiming}

<!-- poner en How long does it take -->


## Population sizes $>10^{10}$ {#pop1010}

(The following examples are commented out for the sake of execution
...
[^errorMF]: We record the largest difference between the computed
