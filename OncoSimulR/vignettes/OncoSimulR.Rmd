...
places of this document; ff you read this section first, you might
...
This table also shows that the `keepEvery = NA` setting, which was in
effect in simulations `exp2` and `mc2`, can make a difference especially
for the McFL models, as seen by the median number of clones and the size
of the returned object. Models `exp2` and `mc2` do not store any
intermediate population samples so clones that become extinct at any
sampling period are pruned and only the existing clones at the end of the
simulation are returned. In contrast, models `exp1` and `mc1` store
population samples at time intervals of 1 (`keepEvery = 1`), even if many
of those clones eventually become extinct. We will return to this issue
below as execution time and object size depend strongly on number of
clones tracked.
...
panderOptions("table.split.table", 99999999)
panderOptions("table.split.cells", Inf)  ## For HTML
...
\@ref(detectprob) and \@ref(bench1xf)). You can verify this by running the
...
#### Exponential, example 4: 50,000 genes, example 2 {#exp50000_2}
...
discard all sampling periods except the very last one (i.e., the
`pops.by.time` matrix contains only the clones with 1 or more cells at the
...
Szendro ("A given genotype may undergo several episodes of colonization
