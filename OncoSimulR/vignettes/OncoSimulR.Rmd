...
and provides some statistics of evolutionary predictability.
...
## 2. Simulate 10 evolutionary trajectories
...
Using as an example @Ochs2015 (we will see this example also in section
...
\@ref(fixation) where we also use this same model). We would then examine
the proportion of genotypes fixed under different scenarios. And we can
extend this example by adding mutator genes:
...
## 3. Simulate evolution in that landscape many times (here just 10)
...
mutator genes are mutated: see section \@(tomlinexcept).)
...
population sizes larger than a pre-specified threshold; for instance,
below we look at increasing population size 4x in a maximum number of 2281
time periods (for real, you would of course increase the number of total
populations, the range of initial population sizes, model, mutation rate,
required population size or number of drivers, etc):
...
pre-specified size, you could run simulations with `onlyCancer = TRUE`,
specifying `detectionSize` and look at the value of `FinalTime` as we did
above (section \@ref(ex-mut-antimut)):
...
from that pair we solve for the value of `carpetbagged`).
