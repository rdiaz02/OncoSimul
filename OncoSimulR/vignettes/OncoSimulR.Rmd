...
by Oncogenetic Trees (OT; [@Desper1999JCB; @Szabo2008]) or Conjunctive
Bayesian Networks (CBN; [@Beerenwinkel2007; @Gerstung2009;
...
other types of restrictions in the accumulation of genes, as in the
"semimonotone" model of @FarahaniLagergren2013 and the XOR models of
@Korsunsky2014. Moreover, different fitness effects related to the order
in which mutations appear can also be incorporated, involving arbitrary
numbers of genes. This is different from "restrictions in the accumulation
of mutations". With order effects, shown empirically in a recent cancer
paper by Ortmann and collaborators [@Ortmann2015], the effect of having
both mutations "A" and "B" differs depending on whether "A" appeared
before or after "B". More generally, now OncoSimulR also allows you to
specify arbitrary epistatic interactions between arbitrary collections of
genes and to model, for example, synthetic mortality or synthetic
viability (again, involving an arbitrary number of genes, some of which
might also depend on other genes, or show order effects with other
genes). Moreover, it is possible to specify the above interactions in
terms of modules, not genes. This idea is discussed in, for example,
@Raphael2014a and @Gerstung2011: the restrictions encoded in, say, CBNs or
OT can be considered to apply not to genes, but to modules, where each
module is a set of genes (and the intersection between modules is the
empty set) that performs a specific biological function. Modules, then,
play the role of a "union operation" over the set of genes in a module. In
addition, arbitrary numbers of genes without interactions (and with
fitness effects coming from any distribution you might want) are also
possible.
...
studies, code to simulate random graphs of the kind often seen in CBN,
OTs, etc, is also available. And OncoSimulR also allows for the generation
...
@Reiter2013a offers somewhat similar functionality to the previous
...
(and, thus, also the TPT of [@Reiter2013a]). We now allow you to
specify all types of fitness effects in other general forward genetic
simulators such as FFPopSim [@Zanini2012], and some that, to our
...
Before anything else, let us load the package. We also explicitly load `r
Biocpkg("graph")` and `r CRANpkg("igraph")` for the vignette to work (you
...
pancreatic cancer poset in Gerstung et al.\ @Gerstung2011 (see more
...
* A special type of epistatic effect that is particularly amenable to
  ...
  is what OT [@Desper1999JCB; @Szabo2008], CBN [@Beerenwinkel2007;
    @Gerstung2009; @Gerstung2011], progression networks
  [@FarahaniLagergren2013], and other similar models
  [@Korsunsky2014] mean. Details are provided in section
...
do in section \@ref(gener-fit-land).
...
### How to specify fitness effects effects with the lego system {#howfit}
...
The original model of McFarland @McFarland2013 has a slightly different
...
In the original McFarland model [@McFarland2013], the effects of
...
$s_{pp} = -s_p/(1 + s_p)$ (see @McFarland2014-phd, his eq. 2.1 in
p.9).  This reparameterization applies to v.2. In v.1 we use the same
parameterization as in the original one in McFarland @McFarland2013.
...
Models based on Bozic, however, have a birth rate of 1
and mutations affect the death rate. In this case, a death rate larger
than birth rate, per se, does not signal immediate extinction and,
moreover, even for death rates that are a few times larger than birth
rates, the clone could mutate before becoming extinct\footnote{We said "a
few times". For a clone of population size 1 ---which is the size at
which all clones start from mutation---, if death rate is, say, 90 but
birth rate is 1, the probability of mutating before becoming extinct is
very, very close to zero for all reasonable values of mutation rate}. How
do we signal immediate extinction or no viability in this case? You can
set the value of $s = -\infty$.
...
## Restrictions in the order of mutations as extended posets {#posetslong}
...
* In relationships of the type used in **Conjunctive** Bayesian
  Networks (CBN) @Gerstung2009, we are modeling an
  **AND** relationship, also called **CMPN** by
  @Korsunsky2014 or **monotone** relationship by
  @FarahaniLagergren2013. If the relationship in the graph is fully
  respected, then "g" will only appear if all of "c", "d", and "e"
  are already mutated.
  
* **Semimonotone** relationships *sensu*
  @FarahaniLagergren2013 or **DMPN** *sensu*
...
more than one incoming connection or more than one parent[^3].

[^3]: OTs and CBNs have some other technical differences about the
...
and in the help for *`poset`*. Here, we continue using two
...
### A first conjunction (AND) example {#cbn1}
...
### A second conjunction example {#cbn2}
...
### A semimonotone or "OR" example {#mn1}
...
module and gene named "Root", as we saw above with  `geneToModule
...
phenomenon such as the one shown empirically by @Ortmann2015: the
...
modules in the `epistasis` component but have no term for
...
## Poset, epistasis, synthetic mortality and viability, order effects and genes without interactions, with some modules {#exlong}
...
We have set `use\_ggrepel = TRUE` to avoid overlap of labels.
...
In the model of Bauer and collaborators [@Bauer2014, p. 54] "For cells
...
expressions. Then, use the `genotFitness` argument to *`allFitnessEffects`*:
...
## Gerstung et al., pancreatic cancer poset {#pancreas}
...
<!-- \clearpage -->
## Raphael and Vandin's modules {#raphael-ex}
...
* **`onlyCancer = TRUE`** A simulation will be repeated until any
  ...
  the simulation reaches `finalTime`\footnote{Of course, the
    "reach cancer" idea and the `onlyCancer` argument are generic
  ...
  only care about one condition, set the other to `NA`.
...
`detectionSize` to "NA" or to huge numbers\footnote{Setting
...
`sampleEvery`\footnote{We assess probability of exiting at every
...
## Numerical issues with Bozic {#ex-0-death}
...
execution. See further comments in \@ref(diffsample).



These are a couple of examples. First we run `oncoSimulPop`
...
*`oncoSimulPop`*) and then sample (in the call to
*`samplePop`*).
...
If you use *`oncoSimulPop`* the arguments for
`detectionSize`, `detectionDrivers` must be the same for all
...
set `removeDirectIndirect = TRUE`? Except for @FarahaniLagergren2013,
