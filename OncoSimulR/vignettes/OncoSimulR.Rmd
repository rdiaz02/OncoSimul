...
studies, code to simulate random graphs of the kind often seen in CBN, OTs,
...
s1 <- oncoSimulPop(10, allFitnessEffects(g1, drvNames = 1:4),
                   ...
                   seed = NULL) ## for repeatability in this vignette
...
---see more details in section \@ref(fixation) where we also use this same
...
no detection can take place unless populations are at least 100 times larger
than the initial population size, and probability of detection is 0.1 with
a poulation size 1,000 times larger than the initial one (`PDBaseline =
5e4`, `n2 = 5e5`). In the other two models (`exp5` and `exp6`), no
detection can take place unless populations are at least 1,000 times larger
than the initial population size, and probability of detection is 0.1 with
a population size 100,000 times larger than the initial one (`PDBaseline =
