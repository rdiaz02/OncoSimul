...
#### Epistatic interactions between drivers and passengers {#bauer}
...
[^mcnum]: Given the dependence of death rates on population size in
    McFarland's model (section \@ref(mcfl) and \@ref(fdfdeath)), if all mutations have
...
So the procedure is to first create a two-column data frame, where
the first column are the genotypes and the second column contains,
as strings, the expressions for the function that relate fitness to
frequencies of other genotypes. We use library
[ExprTk](https://github.com/ArashPartow/exprtk) so the range is
functions you can use is very large
...
```{r hurlbutpay, eval=TRUE,echo=FALSE, fig.cap="Payoff matrix from
Table 2 of Hurlbut et al., 2018, 'Game Theoretical Model of Cancer Dynamics with
Four Cell Phenotypes', *Games*, 9, 61; doi:10.3390/g9030061."}
...
## Frequency-dependent fitness: can I mix relative and aboslute frequencies? {#fdfrelabs}
