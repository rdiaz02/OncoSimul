...
\item When will you stop it: how long to run it, and whether or not to
  ...
  means to reach cancer). This is covered in section \qref{endsimul}.
\end{itemize}
...
OncoSimulR provides very flexible ways to decide how long simulations
should run.


\begin{description}
\item[\texttt{onlyCancer = TRUE}] A simulation will be repeated until the
  first one of the ``reach cancer'' conditions is met before the
  simulation reaches \texttt{finalTime}\footnote{Of
  ...
  \item The tumor is detected according to a stochastic detection
    ...
    with; this is explained below (\qref{detectprob}) and is controlled by
    argument \texttt{detectionProb}.
  \end{itemize}
...
\item[\texttt{onlyCancer = FALSE}] A simulation will run only once, and
  ...
  total population size becomes zero or we reach \texttt{finalTime}.  
\end{description}
...
size of $1e4$ or any clone has four drivers, whichever comes first.
...
conditions has been fulfilled?  The simulation will be repeated again,
...
numbers\footnote{Setting \texttt{detectionDrivers} and
  ...
  respectively.}. In this scenario you simply collect the simulation
output at the end of the run, regardless of what happened with the
population (it became extinct, it did not reach a large size, it did not
accumulate drivers, etc).
...
\texttt{sampleEvery}\footnote{We assess probability of exiting at every
  ...
  assessment by at least \texttt{checkSizePEvery}. For example, suppose
  \texttt{sampleEvery = 2} and \texttt{checkSizePEvery = 3}: we will
  assess exiting at times $4, 8, 12, 16, \ldots$. If \texttt{sampleEvery =
    3} and \texttt{checkSizePEvery = 3}: we will assess exiting at times
...
the following code, that we do not execute here for the sake of speed:
...
will resemble more and more and exponential distribution.
