...
(The meaning of the notation is as follows: ``WT'' denotes the wild-type,
or non-mutated clone. The notation ``x > y'' means that a mutation in
``x'' happened before a mutation in ``y''. A genotype as ``x > y \_ z''
means that a mutation in ``x'' happened before a mutation in ``y''; there
is also a mutation in ``z'', but that is a gene for which order does not
matter).
...
As we have seen before, the stacked plot here is less useful.
