...
par(mfrow = c(1, 2))
plot(ep1, show = "drivers")
plot(ep1, show = "genotypes")
@ 
...
## 3. What genotypes and drivers we get?
plot(ep2, show = "genotypes")
...
also slower than the line plot).

<<>>=
plot(mcfLs, type = "stacked")
@ 
...
## drivers, stacked
plot(sb,show = "drivers", type = "stacked")
## drivers, stream
plot(sb,show = "drivers", type = "stream")
## Genotypes, line plot
plot(sb,show = "genotypes", type = "line")
## genotypes, stacked
plot(sb,show = "genotypes", type = "stacked")
## genotypes, stream
plot(sb,show = "genotypes", type = "stream")
@ 
...
<<>>=
plot(tmp, show = "genotypes")
plot(tmp, show = "genotypes", type = "line")
@ 
...
tmp
plot(tmp, addtot = TRUE, log = "", plotDiversity = TRUE)
## Improve telling appart the most abundant ones sorting colors
...
the \code{streamgraph} package, available from
\Burl{https://github.com/hrbrmstr/streamgraph}.  However, that package is
not available as CRAN or BioConductor package, and thus we cannot depend
on it for this vignette. You can, however, paste the code and make it run
locally.

Before this works, we need to convert the data from the original format in
which it is stored into ``long format''. A simple convenience function is
provided as \Rfunction{OncoSimulWide2Long}.
...
## Stacked area plot for Genotypes
streamgraph(lb1, Genotype, Y, Time, scale = "continuous")
## Stream plot for Drivers
streamgraph(lb1, Drivers, Y, Time, scale = "continuous", offset = "zero")
@ 
