...
The rest of scenarios are obtained under \texttt{onlyCancer = TRUE}. Here
a simulation will be repeated until the first one of the ``reach cancer''
conditions is met\footnote{Of course, the ``reach cancer'' idea and the
  \texttt{onlyCancer} argument are generic names; this could have been
  labeled ``reach cardiovascular disease'' or ``reach whatever interests
...
As an example, focusing on the first two mechanisms, suppose you give
\texttt{detectionSize = 1e4} and \texttt{detectionDrivers = 3} (and you
have \texttt{detectionProb = NA}). A simulation will exit as soon as it
reaches a total population size of $1e4$ or any clone has four drivers,
whichever comes first.


What if we reach \texttt{finalTime} before any of the conditions has been
fulfilled?  The simulation will be repeated again, within limits. These
limits are:
...
probability of detection (or the probability that a sample is taken)
increases with the total population size. This is biologically a
reasonable assumption: the larger the tumor, the more likely it is it will
be detected.

For simplicity, and to make sure the probability is bounded between 0 and
...
where $P(N)$ is the probability that a tumor of total population size $N$
will be detected, $cPDetect$ is the coefficient for the change in
probability with population size, and $PDBaseline$
