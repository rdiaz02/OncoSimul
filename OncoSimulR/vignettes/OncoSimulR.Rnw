...
<<results="hide",message=FALSE, echo=TRUE, include=FALSE>>=
library(OncoSimulR)
library(graph)
library(igraph)
...
an increase/decrease in the mutation rate all over the genome (not unlike
what happens with, say, mutations in mismatch-repair genes or
...
par(mfrow = c(3, 1))
## First, drivers
plot(b1, type = "line", addtot = TRUE, thinData = TRUE)
plot(b1, type = "stacked", thinData = TRUE)
plot(b1, type = "stream", thinData = TRUE)
@ 

<<baux2,fig.width=6.5, fig.height=10>>=
par(mfrow = c(3, 1))
## Next, genotypes
plot(b1, show = "genotypes", type = "line", thinData = TRUE)
plot(b1, show = "genotypes", type = "stacked", thinData = TRUE)
plot(b1, show = "genotypes", type = "stream", thinData = TRUE)
@ 
