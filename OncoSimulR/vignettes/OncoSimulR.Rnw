...
%% FIXME: remove the red thing later
...
%%FIXME : examples of gene-specific mutation rates?
...
core of the code is implemented in C++, providing for fast execution.  To
help with simulation studies, code to simulate random graphs of the kind
often seen in CBN, OTs, etc, is also available. And OncoSimulR also allows
for the generation of random fitness landscapes and the representation of
fitness landscapes.
...
\begin{itemize}
  ...
  \item You can add mutator/antimutator effects.
    \item Fitness and mutation rates can be gene-specific.
    ...
    \item You can generate random fitness landscapes (under the House of
    ...
    \item You can plot fitness landscapes.
...
(and, thus, also the TPT of \cite{Reiter2013a}). We now allow you to
specify all types of fitness effects in other general forward genetic
simulators such as FFPopSim \cite{Zanini2012}, and some that, to our
knowledge (e.g., order effects) are not available from any genetics
simulator. In addition, the ``lego approach'' to flexibly combine
different fitness specifications is also unique.
...
\item Simulate cancer progression: section \ref{simul}. You can simulate
  ...
  \item Decide on a model. This basically amounts to choosing a model with
    ...
    (``Bozic'') or the birth rate (``Exp'')\footnote{It is of course
      ...
      \ref{ex-0-death}}.
  \item Specify other parameters of the simulation. In particular, decide
    when to stop the simulation, mutation rates, etc.
  \end{itemize}
  Of course, at least for initial playing around, you can use the defaults.
  
\item Sample from the simulated data and do something with those simulated
...
##    that will end the simulations, etc
...
\subsection{How long does it take? How fast is OncoSimulR? How much space
  do the return objects take?}
\label{timings}
How long simulations take depend on several factors:

\begin{itemize}
\item Your hardware, of course.
\item The evolutionary model: using the ``McFL'' model is often much
  slower (this model includes density dependence and is more complex).
\item The granularity of how often you keep data (\texttt{keepEvery}
  ...
  times.
\item The mutation rate, because higher mutation rates lead to more
  clones, and more clones mean we need to iterate over, well, more clones,
  and keep larger data structures.

\item The fitness specification (more complex fitness specifications tend
  to be slightly slower).
\item Whether or not you keep the complete clone history (this affects
  mainly size of return object, not speed).
\item The stopping conditions.
\end{itemize}


To give you an idea, we will use the above examples, with the
\texttt{detectionProb} stopping mechanism (see \qref{detectprob}), two
different growth models (exponential and McFarland) and doing 100
simulations. The results I show are for a laptop with an 8-core Intel Xeon
E3-1505M CPU, running Debian GNU/Linux.

<<eval=FALSE>>=

system.time(ep1_exp <- oncoSimulPop(100, sv2, 
...
system.time(ep1_mc <- oncoSimulPop(100, sv2, 
                                  detectionProb = "default", 
                                  detectionSize = NA,
                                  detectionDrivers = NA,
                                  finalTime = NA,
...
system.time(ep2_exp <- oncoSimulPop(100, pancr, 
                                  detectionProb = "default", 
                                  detectionSize = NA,
                                  detectionDrivers = NA,
                                  finalTime = NA,
...
system.time(ep2_mc <- oncoSimulPop(100, pancr, 
                                  detectionProb = "default", 
                                  detectionSize = NA,
                                  detectionDrivers = NA,
                                  finalTime = NA,                                  
...
differ two: in these cases, the simulations using the ``McFL'' model run
...
\textbf{The functionality of version 1 will soon be removed.}
...
\begin{itemize}
\item Combining different types of effects in a single specification. For
  ...
  \textbf{lego system of fitness effects}.
  
  
\item Explicitly passing to OncoSimulR a mapping of genotypes to
  fitness. Here you specify the fitness of each genotype. We will refer to
  this as the \textbf{explicit mapping of genotypes to fitness}.
\end{itemize}
...
\begin{itemize}
\item With the lego system you can specify huge genomes with an enormous
...
\item For many models/data you often intuitively start with the fitness of
...
\item Sometimes you already have a moderate size genotype $\rightarrow$
  fitness mapping and you certainly do not want to do the math by 
  hand: here the lego system would be painful to use.
  
\item But sometimes we do think in terms of ``the effects on fitness of
...
\item If you want to use order effects, you must use the lego system (at
  least for now).
    
\item If you want to specify modules, you must use the lego system (the
  explicit mapping of genotypes is, by its very nature, ill-suited for
  this).
  
\item The lego system might help you see what your model really means: in
...
Regardless of the route, you need to get that information into
  ...
  \Rfunction{allFitnessEffects}: this is the function in charge of reading
  the fitness specifications. We also need to discuss how, what, and where
  you have to pass to \Rfunction{allFitnessEffects}.
 

\subsubsection{Explicit mapping of genotypes to fitness}\label{explicitmap}

Conceptually, the simplest way to specify fitness is to specify the
mapping of all genotypes to fitness explicitly. An example will make this
clear. Let's suppose you have simple two-gene scenario, so a total of four
genotypes, and you have a data frame with genotypes and fitness, where
genoytpes are specified as character vectors, with mutated genes separated
by commas:
...
Now, let's give that to the \Rfunction{allFitnessEffects} function:
...
This way of giving a fitness specification to OncoSimulR might be ideal if
you directly generate random mappings of genotypes to fitness, as we will
do in section \qref{gener-fit-land}.


We will see an example of this way of passing fitness again in
\ref{bauer}, where will compare it with the lego system.

% Please see the documentation of \Rfunction{allFitnessEffects} for further
...
\subsubsection{How to specify fitness effects effects with the lego system}\label{howfit}
...
\subsection{Gene-specific mutation rates}\label{per-gene-mutation}

You can specify gene-specific mutation rates. Instead of passing a scalar
value for \texttt{mu}, you pass a named vector. (This does not work with
...
see file \texttt{./tests/testthat/test.per-gene-mutation-rates.R}).

<<>>=

muvar2 <- c("U" = 1e-6, "z" = 5e-5, "e" = 5e-4, "m" = 5e-3, "D" = 1e-4)
ni1 <- rep(0, 5)
...
\subsection{Mutator genes}\label{mutator}

You can specify mutator/antimutator genes (e.g., \cite{gerrish_complete_2007,
  tomlinson_mutation_1996}): these are genes that, when mutated, lead to
an increase/decrease in the mutation rate all over the genome (similar to what happens with, say, mutations in mismatch-repair genes or
...
You can also use the \texttt{evalAll} functions. We do not show the output
...
\Rfunction{plotClonePhylog} is explained in section \ref{phylog}).
...
\section{Plotting fitness landscapes}
\label{plot-fit-land}


The \Rfunction{evalAllGenotypes} and related functions allow you to obtain
...
%% FIXME use better values, here as I am not being faitfhfull to Bauer's
%% values and figure.
\clearpage
...
\begin{itemize}
\item $\frac{1}{2}(1 + s_p)^k$ when there are $k$ secondary drivers mutated and no primary diver;
\item $\frac{1}{2}\frac{1+S_D^+}{1+S_D^-} (1 + S_{DP})^k$ when the primary driver is mutated;
\end{itemize}
...
And if you compare the tabular output of \Rfunction{evalAllGenotypes} you can
...
\clearpage
An alternative approach to specify the fitness, if the number of genotypes
is reasonably small, is to directly evaluate fitness as given by their
expressions. Then, use the \texttt{genotFitness} argument to \Rfunction{allFitnessEffects}:
...
fitness_bauer <- function(p, s1, s2, s3, s4, 
    ...
    if(!p) {
        ...
        b <- 0.5 * 
            (((1 + sdplus)/(1 + sdminus)  * (1 + sdp)^(sum(c(s1, s2, s3, s4)))))
    }
    fitness <- b - (1 - b)
...
\subsection{Weissman et al., 2009}\label{weissmanex}
...
\clearpage
\section{Running and plotting the simulations: starting, ending, and
  examples}\label{simul}

\subsection{Starting and ending}
\label{sec:starting-ending}
...
\begin{itemize}
\item Where will you start your simulation from. This involves deciding
  the initial population size (argument \texttt{initSize}) and, possibly,
  the genotype of the initial population; the later is covered in section
  \qref{initmut}.

\item When will you stop it: how long to run it, and whether or not to
  require simulations to reach cancer (under some definition of what it
  means to reach cancer). This is covered in \qref{endsimul}.
\end{itemize}
...
## 
## Note we also disable the stopping stochastically
## as a function of size to allow the pop to grow
## large and generate may different clones.
ospI <- oncoSimulPop(4, 
                    o3init, model = "Exp",
                    mu = 5e-5, finalTime = 500,
                    detectionDrivers = 3,
...
\subsection{Ending the simulations}
\label{endsimul}
...
multiple simulations in \qref{sample}.


\begin{description}
\item[\texttt{onlyCancer = TRUE}] A simulation will be repeated until any
  one of the ``reach cancer'' conditions is met, if this happens before
  the simulation reaches \texttt{finalTime}\footnote{Of course, the
    ``reach cancer'' idea and the \texttt{onlyCancer} argument are generic
    names; this could have been labeled ``reach whatever interests
    me''.}. These conditions are:
  \begin{itemize}
  \item Total population size becomes larger than \texttt{detectionSize}.
  \item The number of drivers in any one genotype or clone becomes equal
    to, or larger than, \texttt{detectionDrivers}; note that this allows you
    to stop the simulation as soon as a \textbf{specific genotype} is
    found, by using exactly and only the genes that make that genotype as
    the drivers.
  \item The tumor is detected according to a stochastic detection
    mechanism, where the probability of ``detecting the tumor'' increases
    with population size; this is explained below (\qref{detectprob}) and
    is controlled by argument \texttt{detectionProb}.
  \end{itemize}
  
  As we exit as soon as any of the exiting conditions is reached, if you
  only care about one condition, set the other to \texttt{NA}.

\item[\texttt{onlyCancer = FALSE}] A simulation will run only once, and
  will exit as soon as any of the above conditions are met or as soon as
  the total population size becomes zero or we reach \texttt{finalTime}.
\end{description}


As an example of \texttt{onlyCancer = TRUE}, focusing on the first two
mechanisms, suppose you give \texttt{detectionSize = 1e4} and
\texttt{detectionDrivers = 3} (and you have \texttt{detectionProb =
  NA}). A simulation will exit as soon as it reaches a total population
size of $1e4$ or any clone has four drivers, whichever comes first (if any
of these happen before \texttt{finalTime}).

In the \texttt{onlyCancer = TRUE} case, what happens if we reach
\texttt{finalTime} (or the population size becomes zero) before any of the
``reach cancer'' conditions have been fulfilled?  The simulation will be
repeated again, within the following limits:

\begin{itemize}
\item \texttt{max.wall.time}: the total wall time we allow an individual
  simulation to run;
\item \texttt{max.num.tries}: the maximum number of times we allow a
  simulation to be repeated to reach cancer;
\item \texttt{max.wall.time.total} and \texttt{max.num.tries.total},
  similar to the above but over a set of simulations in function
  \Rfunction{oncoSimulSample}.
\end{itemize}


Incidentally, we keep track of the number of attempts used (the component
\texttt{other$attemptsUsed$}) before we reach cancer, so you can estimate
(as from a negative binomial sampling) the probability of reaching your
desired end point under different scenarios.


The \texttt{onlyCancer = FALSE} case might be what you want to do when you
examine general population genetics scenarios without focusing on possible
sampling issues. To do this, set \texttt{finalTime} to the value you want
and set \texttt{onlyCancer = FALSE}; in addition, set
\texttt{detectionProb} to ``NA'' and \texttt{detectionDrivers} and
\texttt{detectionSize} to ``NA'' or to huge numbers\footnote{Setting
  \texttt{detectionDrivers} and \texttt{detectionSize} to ``NA'' is in
...
% periods (as specified by argument \texttt{finalTime}) or until the
...
% focusing on possible sampling issues. To do this, set \texttt{finalTime}
% to the value you want and set \texttt{onlyCancer = FALSE}; in addition,
% set \texttt{detectionProb} to ``NA'' and \texttt{detectionDrivers} and
% \texttt{detectionSize} to ``NA'' or to impossibly large
% numbers\footnote{Setting \texttt{detectionDrivers} and
%   \texttt{detectionSize} to ``NA'' is in fact equivalent to setting them
%   to the largest possible numbers: $2^32 -1$ and $\infty$,
%   respectively.}. This scenario is the \texttt{onlyCancer = FALSE} because
...
% If we set \texttt{onlyCancer = TRUE}. Here a simulation will be repeated
% until the first one of the ``reach cancer'' conditions is met\footnote{Of
%   course, the ``reach cancer'' idea and the \texttt{onlyCancer} argument
...
% \item total population size, given by \texttt{detectionSize};

% \item number of drivers in any one genotype or clone, given by
%   \texttt{detectionDrivers}; note that this allows you to stop the
%   simulation as soon as a \textbf{specific genotype} is found, by using
...
%   \texttt{detectionProb}; this is explained below: \qref{detectprob}.
...
\subsubsection{Stochastic detection mechanism: \texttt{detectionProb}}
\label{detectprob}

This is the process that is controlled by the argument
\texttt{detectionProb}. Here the probability of tumor detection increases
...
\texttt{initSize}) and we model the increase in $P(N)$ as a function of
...
\texttt{checkSizePEvery}, which will often be much larger than
\texttt{sampleEvery}\footnote{We assess probability of exiting at every
  sampling time, as given by \texttt{sampleEvery}, that is the smallest
  possible sampling time that is separated from the previous time of
  assessment by at least \texttt{checkSizePEvery}. In other words, the
  interval between successive assessments will be the smallest multiple
  integer of \texttt{sampleEvery} that is larger than
  \texttt{checkSizePEvery}. For example, suppose \texttt{sampleEvery = 2}
  and \texttt{checkSizePEvery = 3}: we will assess exiting at times
  $4, 8, 12, 16, \ldots$. If \texttt{sampleEvery = 3} and
  \texttt{checkSizePEvery = 3}: we will assess exiting at times
  $6, 12, 18, \ldots$.} Biologically, a way to think of
\texttt{checkSizePEvery} is ``time between doctor appointments''.


Finally, you can specify $cPDetect$ directly (you will need to set
\texttt{n2} and \texttt{p2} to NA). However, it might be more intuitive to
specify the pair \texttt{n2}, \texttt{p2}, such that $P(n2) = p2$ (and
from that pair we solve for the value of \texttt{cPDetect}).
...
As you decrease \texttt{checkSizePEvery} the distribution of ``FinalTime''
...
in \qref{s-cbn1} and \qref{mcf5070}, with the default arguments.


% Further comments about this procedure and its relation to sampling
% multiple simulations are provided in \qref{popss-osp}.


% \subsubsection{So does \texttt{finalTime} matter much?}
% \label{meaning-final-time}

% This should be implicit from the above discussion, but \texttt{finalTime}
...
The key function here is \Rfunction{samplePop}, either called explicitly
after \Rfunction{oncoSimulPop} (or \Rfunction{oncoSimulIndiv}), or
implicitly as part of a call to \Rfunction{oncoSimulSample}. With
\Rfunction{samplePop} you can use \textbf{single cell} or \textbf{whole
  tumor} sampling (for details see the help of
\Rfunction{samplePop}). Depending on how the simulations were conducted,
...
\paragraph{You want to keep the complete history of simulations} You will
simulate using:

\begin{itemize}
\item \texttt{oncoSimulIndiv} repeatedly (maybe within
  \Rfunction(mclapply), to parallelize the run).
\item \texttt{oncoSimulPop}. \texttt{oncoSimulPop} is basically a
  thin wrapper around \texttt{oncoSimulIndiv} that uses
  \Rfunction{mclapply}.
\end{itemize}
In both cases, you specify the conditions for ending the simulations (as
explained in \qref{endsimul}).

Then, you will use function \texttt{oncoSimulSample} to obtain the matrix
of samples by mutations.

  
\paragraph{You do not want to keep the complete history of the
  simulations} You will simulate using:

\begin{itemize}
\item \texttt{oncoSimulIndiv} repeatedly,  with argument \texttt{keepEvery = NA}. 
\item \texttt{oncoSimulPop}, with argument \texttt{keepEvery = NA}. 
\end{itemize}
In both cases you specify the conditions for ending the simulations (as
explained in \qref{endsimul}).  Then, you will use function
\texttt{oncoSimulSample}.

\begin{itemize}
  
\item \texttt{oncoSimulSample}, specifying the conditions for ending the
  simulations (as explained in \qref{endsimul}). In this case, you will
  not use \texttt{oncoSimulSample}, as that is implicitly called by
  \texttt{oncoSimulSample}.  The output is directly the matrix (and a
  little bit of summary from each run), and during the simulation it
  only stores one time point.
  
\end{itemize}
% In this case, you will not use \texttt{oncoSimulSample}, as that is
% implicitly called by \texttt{oncoSimulSample}.
...
the call to \Rfunction{oncoSimulPop}) and then sample (in the call to
\Rfunction{samplePop}). Other reasons for choosing one over the other
might have to do with flexibility (e.g., if you use
\Rfunction{oncoSimulPop} the arguments for \texttt{detectionSize},
\texttt{detectionDrivers} must be the same for all simulations but this is
not the case for \Rfunction{oncoSimulSample}) and parallelized
execution. See further comments in \qref{diffsample}.



These are a couple of examples. First we run \texttt{oncoSimulPop}
to obtain 4 simulations and in the last line we sample from them:
<<>>=
...
have done instead (note the argument \texttt{keepEvery = NA})
...
Or we could have used \Rfunction{oncoSimulSample}:
...
Why the above differences? If we are only interested in the final matrix
of populations by mutations, keeping the complete history the above is
wasteful, because we store fully all of the simulations (in the call to
\Rfunction{oncoSimulPop}) and then sample (in the call to
\Rfunction{samplePop}).
...
\subsection{Further criteria for choosing between sampling
  procedures}\label{choose-proc}

If you use \Rfunction{oncoSimulPop} the arguments for
\texttt{detectionSize}, \texttt{detectionDrivers} must be the same for all
simulations. This is not the case for \Rfunction{oncoSimulSample}.


\Rfunction{samplePop} can only sample with the arguments you give
(especially )
...
or different for each simulation (with argument \texttt{popSizeSample}).
...
An advantage of \Rfunction{oncoSimulSample} is that we can specify
...
% \subsubsection{\texttt{popSizeSample} and \Rfunction{oncoSimulPop}}
% \label{popss-osp}


% Suppose you run a set of simulations with a large \texttt{finalTime} with
% \Rfunction{oncoSimulPop}. Since you can pass to \Rfunction{samplePop} a
% vector for \texttt{popSizeSample} this might seem to resemble a
% deterministic version of the stochastic detection mechanism
% (\qref{detectprob}): for each simulation, you would take a sample at the
...
% 1000. Now, if you pass a vector of \texttt{popSizeSample} that has a value
...
\subsection{Whole tumor sampling, genotypes, and allele counts: what
  gives?}\label{wtsampl}
...
\section{Showing the relationships of clones}\label{phylog}
...
We can plot the parent-child relationships\footnote{There are several
...
% pdf("phylog-clone1.pdf", width = 4, height = 4)
% par(cex = 0.5); plotClonePhylog(mcf1s, N = 1, t = c(900, 1000), timeEvents = TRUE)
% dev.off()
...
\subsection{Parent-child relationships from multiple runs}\label{phylogmult}

If you use \Rfunction{oncoSimulPop} you can store and plot the
...
\section{Generating fitness landscapes}
\label{gener-fit-land}


It is possible to generate mappings of genotype to fitness using the
function \Rfunction{rfitness} that allows you to use from a pure House of
Cards model to a purely additive model. I have followed
\cite{szendro_quantitative_2013, franke_evolutionary_2011} and model
fitness as
...
positions mutated by setting \texttt{reference = ``max''}, or use the
wildtype by using a string of 0s, or randomly select a genotype as a
reference by using \texttt{reference = ``random''}.  And by playing with $c$
...
similar to the one on \cite{greene_changing_2014} if you use
\Rfunction{rfitness} with the argument \texttt{reference = ``max''}.


What can you do with these genotype to fitness mappings? You could plot
them, you could use them as input for \Rfunction{oncoSimulIndiv} and
related functions, or you could export them and plot them externally
(e.g., in MAGELLAN: \url{http://wwwabi.snv.jussieu.fr/public/Magellan/},
\cite{brouillet_magellan:_2015}).
...
## A 5-gene example, where the reference genotype is the one 
## the one with all positions mutated, similar to Greene and Crona.
## We will plot the landscape and use it for simulations
...
% pdf(file = "fl1.pdf", height = 8, width = 5)
% plot(r1)
% dev.off()
