...
\newcommand{\red}[1]{{\textcolor {red} {NEW: #1}}}
...
\begin{itemize}
  ...
  \item You can add passenger mutations.
    ...
    \item Possibility to generate random fitness landscapes (under the
      House of Cards, Rough Mount Fuji, or additive models, or
      combinations of the former) and use those landscapes as input to the
      simulation functions.
      
    \item Plotting of fitness landscapes.
...
\begin{itemize}
\item \red{Combining different types of effects in a single specification. For
...
\item \red{Explicitly passing to OncoSimulR a mapping of genotypes to
...
\item Sometimes you already have a moderate size genotype $\rightarrow$
  fitness mapping and \ldots you certainly do not want to do the math by
  hand and here the lego system would be painful to use.
...
\red{Regardless of the route, you need to get that information into
...
\subsubsection{\red{Explicit mapping of genotypes to fitness}}\label{explicitmap}
...
\red{We will see below that with the ``lego system'' it is possible to do
something very similar to that the explicit mapping of section
\ref{explicitmap}.  But this will sometimes require a more cumbersome
notation (and sometimes requiring you to do some math). We will see
...
\subsection{\red{Gene-specific mutation rates}}\label{per-gene-mutation}

It is now possible to specify gene-specific mutation rates. Instead
of passing a scalar value for \texttt{mu}, you pass a named vector. (This
does not work with the old v.\ 1 format, though; yet another reason to
stop using that format). This is a simple example (many more are available
in the tests, see file \texttt{/tests/testthat/test.per-gene-mutation-rates.R}).
...
\subsection{\red{Mutator genes}}\label{mutator}

It is now possible to specify mutator genes (FIXMEzz: cite some paper,
e.g., Tomlinson or Gerrish): these are genes that, when mutated, lead to
an increase in the mutation rate all over the genome (not unlike what
happens with, say, microsatellite instability in cancer ---FIXME: cite
Vogelstein/Nowak).

The specification is very similar to that for fitness effects, except we
do not (at least for now) use DAGs for poset-like structures nor allow for
order effects (we have seen no reference in the literature to suggest any
of these would be relevant). You can, however, specify epistasis and use
modules. Note that the mutator genes must be a subset of the genes in the
fitness effects; if you want to have mutator genes that have no direct
fitness effects, give them a fitness effect of 0.
...
\section{\red{Plotting fitness landscapes}}
\label{plot-fit-land}
...
wb <- allFitnessEffects(
    epistasis = c(
        ...
        "B:C" = s2,
        "A:B:C" = s3))
plotFitnessLandscape(wb, use_ggrepel = FALSE) ## use ggrepel to avoid overlap of labels
...
\section{\red{Generating fitness landscapes}}
...
number of positions that differ) and $x_i$ is a random variable (in this
