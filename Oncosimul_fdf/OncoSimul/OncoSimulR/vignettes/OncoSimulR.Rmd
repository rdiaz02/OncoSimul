...
* From version 2.9.10, you can use simulations in a frequency dependent fitness 
  way, that is, puting the fitness specifications not only as fixed values, but 
  also as functions of the frequecies of the clones, and evaluating frequently the 
  fitness across the simulation, you can obtein a totaly dynamic fitness landscape 
  (see section \@ref(FDF)).
...
### Simulating trayectories employing the avalalaible models with frequency 
    dependent fitness 
    
The new frequency dependent fitness funcionality allows users to run simulations 
in a different way, defining the fitness effects as funntions of clone's frequencies.
This point of view comes from frequency dependent selection contex in Evolutionary 
Biology and in the framework of this package is employed using stochastic models.
...
depending the model used, must be maintained updated. To do this is
mandatory to use a short step to reevaluate fitness, what is done using a
...
(<http://www.partow.net/programming/exprtk/>) that is used to parse an evaluate the
fitness ecuations. The numeric vector `spPopSizes` is only necesary to evaluate
...
which variables are the relative frequencies of the clones in the tumour population.
In order to specify this properly, you need to build a `allFitnessEffects` object,
...
corresponding warnings that advert you of the transformation. If you dont want to
see this sometimes ugly messages, just use `stringsAsFactors = FALSE` in data frame
definition. Setting the paramenter `frequencyDependentFitness` to TRUE, you are
fixing that all downstream works are going to be done in a frequency Dependent
Fitness mode.

The case of the example above is inspired in @Axelrod2006, where all clones
...
fitness effects as linear functions, where the fitness increase with the number of
...
get a high frequency, WT decreases fast because its frequency fell down and
...
dominates the entire population quickly. If we think in this double mutant as the
...
But we can be worried about what could happen if the simulation is repeated a bunch
of times, or in other words, how stable is this result under this conditions? 
...
and can be apreciated that the final population composition under this situation is really stable and produce the same results with little variability.
